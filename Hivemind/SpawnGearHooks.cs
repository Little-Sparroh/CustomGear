...
///
/// Strategy:
///  1. Prefix remaps allGearIndex to the vanilla SwarmGun index.
///  2. Postfix stamps our GearInfo + HiveLauncherBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[HiveLauncher] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[HiveLauncher] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[HiveLauncher] Cannot remap spawn — base SwarmGun AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        SparrohPlugin.Logger?.LogInfo(
            $"[HiveLauncher] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        if (baseIndex >= 0 && baseIndex < Global.Instance.AllGear.Length)
        {
            ...
            if (at != null && !IsOurCatalogGear(at) && at is SwarmGun)
                return baseIndex;
        }
        ...
        // Prefer a real SwarmGun that is not our catalog entry.
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
            if (Global.Instance.AllGear[i] is SwarmGun &&
                !IsOurCatalogGear(Global.Instance.AllGear[i]))
            {
                WeaponRegistration.SetBaseAllGearIndex(i);
                return i;
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[HiveLauncher] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[HiveLauncher] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[HiveLauncher] Catalog gear missing during stamp.");
            return;
        }
    ...
    /// <summary>
    /// After NGO spawns vanilla SwarmGun, rebind live instance to catalog identity
    /// and re-run ApplyUpgrades so GunData restores from our Hive baseline.
    /// </summary>
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            HiveLauncherBehaviour behaviour = live.gameObject.GetComponent<HiveLauncherBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<HiveLauncherBehaviour>();

            HiveLauncherBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<HiveLauncherBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[HiveLauncher] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        // Ensure Hive balance sticks even if ApplyUpgrades path is partial.
        if (live is Gun liveGun && catalog is Gun catalogGun)
        {
            // Catalog damage is 34; if live still looks like a different baseline, re-apply.
            if (Mathf.Abs(liveGun.GunData.damage - catalogGun.GunData.damage) > 0.01f ||
                liveGun.GunData.magazineSize != catalogGun.GunData.magazineSize)
            {
                WeaponRegistration.ApplyHiveLauncherStats(liveGun);
            }

            var hiveLive = liveGun.GetComponent<HiveLauncherBehaviour>();
            if (hiveLive != null)
                hiveLive.OnUpgradesApplied(liveGun);

            HiveLauncherHudInstaller.EnsureLiveHud(liveGun);
        }
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[HiveLauncher] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[HiveLauncher] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[HiveLauncher] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[HiveLauncher] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[HiveLauncher] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[HiveLauncher] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            if (baseIndex < 0)
            {
                ...
                {
                    if (Global.Instance.AllGear[g] is SwarmGun &&
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[HiveLauncher] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[HiveLauncher] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[HiveLauncher] OnClose stamp: {ex.Message}");
            }
        }
