...
/// <summary>
/// Hive Launcher — custom primary for Mycopunk.
/// Phase 0/1: register a SwarmGun clone with isolated GearInfo + base balance.
/// Vanilla Swarm Launcher is left unmodified.
/// </summary>
[BepInPlugin(PluginGUID, PluginName, PluginVersion)]
[MycoMod(null, ModFlags.IsSandbox)]
public class SparrohPlugin : BaseUnityPlugin
{
    public const string PluginGUID = "sparroh.hivemind";
    public const string PluginName = "Hivemind";
    public const string PluginVersion = "1.0.0";

    /// <summary>Stable numeric GearInfo.ID — high unique range.</summary>
    public const int GearId = 91900;

    /// <summary>Value of GearInfo.APIName — used by FindGear / AllGear scans.</summary>
    public const string GearApiName = "hive_mind";

    public const string GearDisplayName = "Hivemind";
    public const string GearDescription =
        "Living pellet swarm primary. Plant a hovering cloud, then release to dive. " +
        "Full-auto baseline — grow the hive later.";

    /// <summary>
    /// Must be SwarmGun: SwarmBullet hard-casts ParentSource to SwarmGun for hover/dive.
    /// </summary>
    public const string BaseTypeName = "SwarmGun";
    ...
    private void Awake()
    {
        ...
        HiveLauncherHudHooks.Apply(_harmony);
    ...
    internal void TryRegisterGear(string reason)
    {
        ...
        {
            Logger.LogDebug($"[HiveLauncher] Global.AllGear not ready yet ({reason}).");
            return;
        }
        ...
        try
        {
            ...
            Logger.LogInfo(
                $"[HiveLauncher] Registered gear '{GearDisplayName}' " +
                $"(api={GearApiName}, id={GearId}) via {reason}.");
        }
        catch (Exception ex)
        {
            Logger.LogError($"[HiveLauncher] Gear registration failed: {ex}");
        }
    }
...
/// <summary>
/// Keep Hive Launcher alive across save load.
///
...
internal static class PlayerDataPersistenceHooks
{
    ...
    private static void Postfix()
    {
        try
        {
            ...
            if (gear == null)
            {
                SparrohPlugin.Logger?.LogWarning("[HiveLauncher] Persistence: gear missing after OnAwake.");
                return;
            }
            ...
            if (gd != null)
            {
                ...
                SparrohPlugin.Logger?.LogInfo(
                    $"[HiveLauncher] Persistence OK: level={gd.Level} unlocked={gd.IsUnlocked} " +
        ...
        {
            SparrohPlugin.Logger?.LogError($"[HiveLauncher] Persistence postfix failed: {ex}");
        }
    }
    ...
    private static void TryEnsureSavedLoadoutPointsAtCatalog(IUpgradable gear)
    {
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[HiveLauncher] Save loadout references catalog id={id} " +
