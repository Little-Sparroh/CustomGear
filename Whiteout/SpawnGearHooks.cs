...
///
/// Strategy:
///  1. Prefix with <c>ref int allGearIndex</c> so Harmony remaps to BounceShotgun base.
///  2. Postfix: stamp GearInfo + WhiteoutBehaviour onto the live instance.
///  3. Null-guard GearSelectionWindow OnOpen / OnCloseCallback.
/// </summary>
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[Whiteout] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        SparrohPlugin.Logger?.LogDebug(
            $"[Whiteout] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[Whiteout] Cannot remap spawn — base gun AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        SparrohPlugin.Logger?.LogDebug(
            $"[Whiteout] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        // Prefer BounceShotgun specifically.
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
            if (Global.Instance.AllGear[i] is BounceShotgun bounce &&
                !IsOurCatalogGear(bounce))
            {
                WeaponRegistration.SetBaseAllGearIndex(i);
                return i;
            }
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[Whiteout] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[Whiteout] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[Whiteout] Catalog gear missing during stamp.");
            return;
        }
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            WhiteoutBehaviour behaviour = live.gameObject.GetComponent<WhiteoutBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<WhiteoutBehaviour>();

            WhiteoutBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<WhiteoutBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[Whiteout] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        if (live is Gun liveGun)
        {
            if (catalog is Gun catalogGun && NeedsStatRepair(liveGun, catalogGun))
                WeaponRegistration.ApplyWhiteoutStats(liveGun, SparrohPlugin.Logger);
            else
                WeaponRegistration.ApplyWhiteoutStats(liveGun, SparrohPlugin.Logger);

            liveGun.IsAimEnabled = WhiteoutBalance.IsAimEnabled;
            liveGun.GunData.automatic = WhiteoutBalance.Automatic;
            liveGun.GunData.useAmmoOnFire = WhiteoutBalance.UseAmmoOnFire;

            if (WhiteoutBehaviour.TryGet(liveGun, out var wo))
                wo.OnUpgradesApplied(liveGun);
        ...
        SparrohPlugin.Logger?.LogDebug(
            $"[Whiteout] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
    ...
    private static bool NeedsStatRepair(Gun live, Gun catalog)
    {
        ...
        if (catalog.GunData.magazineSize == WhiteoutBalance.MagazineSize &&
            live.GunData.magazineSize != WhiteoutBalance.MagazineSize)
            return true;

        if (live.IsAimEnabled != WhiteoutBalance.IsAimEnabled)
            return true;

        if (live.GunData.useAmmoOnFire != WhiteoutBalance.UseAmmoOnFire)
            return true;
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogDebug(
                $"[Whiteout] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[Whiteout] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[Whiteout] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[Whiteout] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[Whiteout] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            if (baseIndex < 0)
            {
                ...
                {
                    if (Global.Instance.AllGear[g] is BounceShotgun bounce &&
                        !SpawnGearHooks.IsOurCatalogGear(bounce))
                    {
                        baseIndex = g;
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[Whiteout] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogDebug(
                $"[Whiteout] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[Whiteout] OnClose stamp: {ex.Message}");
            }
        }
