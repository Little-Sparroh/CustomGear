...
/// <summary>
/// Creates and registers Hooklash as a GearType.Melee kit by cloning vanilla MeleeGear.
...
public static class WeaponRegistration
{
        ...
        out IUpgradable registeredGear)
    {
        ...
        if (existing != null)
        {
            ...
            if (CatalogMelee != null)
                ApplyHooklashStats(CatalogMelee);
        ...
        HooklashBehaviour behaviour = clone.GetComponent<HooklashBehaviour>();
        if (behaviour == null)
            behaviour = clone.AddComponent<HooklashBehaviour>();
        behaviour.InitializeAsPrefab(description);
        behaviour.CapturePrefabSnapshot();

        ApplyHooklashStats(cloneMelee);
    ...
    public static void ApplyHooklashStats(MeleeGear melee)
    {
        if (melee == null)
            return;

        ref GunData gun = ref melee.GunData;
        gun.damage = HooklashBalance.Damage;
        gun.damageEffect = HooklashBalance.DamageEffect;
        gun.damageEffectAmount = HooklashBalance.DamageEffectAmount;
        gun.bulletMagnetismTarget = HooklashBalance.Size;
        gun.hitForce = HooklashBalance.HitForce;
        gun.hitVFXSize = HooklashBalance.HitVfxSize;
        HooklashBalance.ApplyReach(ref gun, HooklashBalance.Reach);

        // No ammo / no reload pressure on melee kit.
        gun.hasLimitedAmmo = HooklashBalance.HasLimitedAmmo;
        gun.magazineSize = HooklashBalance.MagazineSize;
        gun.ammoCapacity = HooklashBalance.AmmoCapacity;
        gun.autoReloadWhenEmpty = HooklashBalance.AutoReloadWhenEmpty;
        gun.refillAmmoOnReload = HooklashBalance.RefillAmmoOnReload;
        gun.useAmmoOnFire = HooklashBalance.UseAmmoOnFire;

        ref CooldownData cd = ref melee.CooldownData;
        cd.rechargeDuration = Mathf.Max(0.05f, HooklashBalance.Cooldown);
    }
