...
/// <summary>
/// Creates and registers Aussie Special by cloning BounceShotgun and applying hopper stats.
/// </summary>
public static class WeaponRegistration
{
        ...
        out IUpgradable registeredGear)
    {
        ...
        if (existing != null)
        {
            ...
            if (existing is Gun existingGun)
                ApplyAussieSpecialStats(existingGun, log);
        ...
        AussieSpecialBehaviour behaviour = clone.GetComponent<AussieSpecialBehaviour>();
        if (behaviour == null)
            behaviour = clone.AddComponent<AussieSpecialBehaviour>();
        behaviour.InitializeAsPrefab(description);

        ApplyAussieSpecialStats(cloneGun, log);
    ...
    /// <summary>
    /// Hopper baseline: mag 2, semi, no ADS, 6 pellets, 1 bounce, ~0.3s interval.
    /// Damage / reserve / spread inherit BounceShotgun unless balance overrides are set.
    /// </summary>
    public static void ApplyAussieSpecialStats(Gun gun, ManualLogSource log = null)
    {
        if (gun == null)
            return;
        ...
        if (AussieSpecialBalance.DamageOverride > 0f)
            g.damage = AussieSpecialBalance.DamageOverride;

        g.damageEffect = AussieSpecialBalance.DamageEffect;
        g.damageEffectAmount = AussieSpecialBalance.DamageEffectAmount;
        g.fireInterval = AussieSpecialBalance.FireInterval;
        g.automatic = AussieSpecialBalance.Automatic;
        g.bulletsPerShot = AussieSpecialBalance.BulletsPerShot;
        g.burstSize = AussieSpecialBalance.BurstSize;
        g.burstFireInterval = AussieSpecialBalance.BurstFireInterval;
        g.useAmmoOnFire = AussieSpecialBalance.UseAmmoOnFire;
        g.doesEachBulletInShotRemoveAmmo = AussieSpecialBalance.DoesEachBulletInShotRemoveAmmo;
        g.doesEachBulletInShotTriggerEffects = AussieSpecialBalance.DoesEachBulletInShotTriggerEffects;

        g.magazineSize = AussieSpecialBalance.MagazineSize;
        g.hasLimitedAmmo = AussieSpecialBalance.HasLimitedAmmo;
        g.ammoCapacity = AussieSpecialBalance.AmmoCapacity;
        g.refillAmmoOnReload = AussieSpecialBalance.RefillAmmoOnReload;
        g.autoReloadWhenEmpty = AussieSpecialBalance.AutoReloadWhenEmpty;
        if (AussieSpecialBalance.ReloadDurationOverride > 0f)
            g.reloadDuration = AussieSpecialBalance.ReloadDurationOverride;

        g.maxBounces = Mathf.Max(g.maxBounces, AussieSpecialBalance.MaxBounces);
        if (AussieSpecialBalance.BulletSpeedOverride > 0f)
            g.bulletSpeed = AussieSpecialBalance.BulletSpeedOverride;

        // Dual-trigger identity: no ADS.
        gun.IsAimEnabled = AussieSpecialBalance.IsAimEnabled;

        // Behaviour defaults (catalog + live).
        if (gun.gameObject != null)
        {
            AussieSpecialBehaviour b = gun.gameObject.GetComponent<AussieSpecialBehaviour>();
            if (b != null)
            {
                b.WeaponData.preBounceDamageMult = AussieSpecialBalance.PreBounceDamageMult;
                b.WeaponData.barrelFireInterval = AussieSpecialBalance.BarrelFireInterval;
                b.WeaponData.chamberSizeLeft = AussieSpecialBalance.ChamberSizeLeft;
                b.WeaponData.chamberSizeRight = AussieSpecialBalance.ChamberSizeRight;
            }
        }

        //log?.LogInfo(
            //$"[WeaponRegistration] Applied Aussie Special stats: dmg={g.damage}, " +
            //$"interval={g.fireInterval}, chambers={AussieSpecialBalance.ChamberSizeLeft}|{AussieSpecialBalance.ChamberSizeRight}, " +
            //$"magSum={g.magazineSize}, reserve={g.ammoCapacity}, pellets={g.bulletsPerShot}, " +
            //$"bounces={g.maxBounces}, aim={gun.IsAimEnabled}, preBounce×={AussieSpecialBalance.PreBounceDamageMult}.");
        ...
        ManualLogSource log)
    {
        ...
        // Isolate upgrade pool from Jackrabbit — empty until Phase 2+.
...
/// <summary>Placeholder for future mesh / VFX swaps. Uses BounceShotgun visuals for now.</summary>
public static class ModelImportHooks
{
    public static void ApplyPlaceholderHooks(GameObject gearRoot, ManualLogSource log)
    {
        log?.LogDebug("[ModelImportHooks] Placeholder — using vanilla BounceShotgun visuals.");
    }
