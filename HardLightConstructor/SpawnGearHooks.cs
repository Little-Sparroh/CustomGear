...
///  2. Postfix stamps our GearInfo + HardLightConstructorBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[HardLightConstructor] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[HardLightConstructor] Cannot remap spawn — base gun AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        SparrohPlugin.Logger?.LogInfo(
            $"[HardLightConstructor] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        // Prefer CartridgeSMG (Cycler) specifically.
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
            if (Global.Instance.AllGear[i] is CartridgeSMG &&
                !IsOurCatalogGear(Global.Instance.AllGear[i]))
            {
                WeaponRegistration.SetBaseAllGearIndex(i);
                return i;
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[HardLightConstructor] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[HardLightConstructor] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[HardLightConstructor] Catalog gear missing during stamp.");
            return;
        }
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            HardLightConstructorBehaviour behaviour = live.gameObject.GetComponent<HardLightConstructorBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<HardLightConstructorBehaviour>();

            HardLightConstructorBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<HardLightConstructorBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[HardLightConstructor] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        if (live is Gun liveGun && catalog is Gun catalogGun)
        {
            if (NeedsStatRepair(liveGun, catalogGun))
                WeaponRegistration.ApplyHlcStats(liveGun, SparrohPlugin.Logger);

            HardLightConstructorBehaviour.ApplyDesignLocks(liveGun);

            if (HardLightConstructorBehaviour.TryGet(liveGun, out var hlcLive))
                hlcLive.OnUpgradesApplied(liveGun);
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[HardLightConstructor] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
    ...
    private static bool NeedsStatRepair(Gun live, Gun catalog)
    {
        ...
        if (catalog.GunData.damageEffect == HlcBalance.DamageEffect &&
            live.GunData.damageEffect != HlcBalance.DamageEffect)
            return true;

        if (Mathf.Abs(live.GunData.damage - catalog.GunData.damage) > 0.5f &&
            catalog.GunData.damage >= 15f && live.GunData.damage < 12f)
            return true;

        if (live.GunData.magazineSize != catalog.GunData.magazineSize &&
            catalog.GunData.magazineSize == HlcBalance.MagazineSize)
            return true;
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[HardLightConstructor] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[HardLightConstructor] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[HardLightConstructor] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[HardLightConstructor] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[HardLightConstructor] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            if (baseIndex < 0)
            {
                ...
                {
                    if (Global.Instance.AllGear[g] is CartridgeSMG &&
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[HardLightConstructor] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[HardLightConstructor] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[HardLightConstructor] OnClose stamp: {ex.Message}");
            }
        }
