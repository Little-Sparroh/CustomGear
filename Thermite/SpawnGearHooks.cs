...
/// <summary>
/// Fixes equipping runtime-cloned Thermite.
///
/// Strategy:
///  1. Prefix remaps our catalog AllGear index → vanilla IncendiaryGrenade index (ref param).
///  2. Postfix stamps Thermite identity onto the live IncendiaryGrenade instance (with retry).
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            ThermitePlugin.Logger?.LogError("[Thermite] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        //ThermitePlugin.Logger?.LogInfo(
            //$"[Thermite] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            ThermitePlugin.Logger?.LogError(
                "[Thermite] Cannot remap spawn — base IncendiaryGrenade AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        ThermitePlugin.Logger?.LogInfo(
            $"[Thermite] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        if (baseIndex >= 0 && baseIndex < Global.Instance.AllGear.Length)
        {
            ...
            if (at != null && !IsOurCatalogGear(at) && at is IncendiaryGrenade)
                return baseIndex;
        ...
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
            if (Global.Instance.AllGear[i] is IncendiaryGrenade &&
                !IsOurCatalogGear(Global.Instance.AllGear[i]))
            {
                GrenadeRegistration.SetBaseAllGearIndex(i);
                return i;
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        try
        {
            ...
            {
                ThermitePlugin plugin = ThermitePlugin.Instance;
                if (plugin != null)
        ...
        {
            ThermitePlugin.Logger?.LogError($"[Thermite] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static IEnumerator StampWhenReady(Player player, int slot)
    {
        for (int attempt = 0; attempt < 30; attempt++)
        {
            ...
            {
                ThermitePlugin.Logger?.LogWarning(
                    $"[Thermite] Deferred stamp attempt {attempt}: {ex.Message}");
            }
        }

        ThermitePlugin.Logger?.LogWarning(
            $"[Thermite] Deferred stamp gave up on Gear[{slot}] after retries.");
    }
    ...
    private static bool TryStampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        bool alreadyIdentity =
            live.Info != null && live.Info.APIName == ThermitePlugin.GearApiName &&
            live.gameObject != null && live.gameObject.GetComponent<ThermiteBehaviour>() != null;

        IUpgradable catalog = ThermitePlugin.CustomGrenadePrefab
                              ?? GrenadeRegistration.CatalogGear
                              ?? FindCatalogInAllGear();

        if (catalog == null)
        {
            ThermitePlugin.Logger?.LogWarning("[Thermite] Catalog gear missing during stamp.");
            return false;
        }
        ...
        if (!alreadyIdentity)
        {
            ...
            {
                ThermiteBehaviour behaviour = live.gameObject.GetComponent<ThermiteBehaviour>();
                if (behaviour == null)
                    behaviour = live.gameObject.AddComponent<ThermiteBehaviour>();

                ThermiteBehaviour templateBehaviour = null;
                if (catalog is Component cc)
                    templateBehaviour = cc.GetComponent<ThermiteBehaviour>();
        ...
        // Always stamp ThermiteBalance onto the live IncendiaryGrenade instance before
        // upgrades scale it — spawn remaps to vanilla Incendiary prefab numbers otherwise.
        ...
        {
            ThermitePlugin.Logger?.LogWarning(
                $"[Thermite] Post-stamp ApplyUpgrades failed on Gear[{slot}]: {ex.Message}");
        }

        // Do NOT wipe gimmicks after Apply — upgrades intentionally set them.
        if (ThermiteBehaviour.TryGet(live, out ThermiteBehaviour thermiteBehaviour))
        {
            thermiteBehaviour.SyncToVanillaIncendiary(live);
            ThermitePlayerHooks.EnsureBound(live);
        }
        ...
        ThermitePlugin.Logger?.LogInfo(
            $"[Thermite] Stamped custom identity on Gear[{slot}] ({ThermitePlugin.GearApiName}).");
        return true;
    }
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            GrenadeRegistration.EnsureGearData(catalog, autoUnlock: true, ThermitePlugin.Logger);
            ...
            ThermitePlugin.Logger?.LogInfo(
                $"[Thermite] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            ThermitePlugin.Logger?.LogWarning($"[Thermite] PersistEquippedCatalogId: {ex.Message}");
        }
    }
    ...
    internal static bool IsOurCatalogGear(IUpgradable gear)
    {
        ...
        if (gear == ThermitePlugin.CustomGrenadePrefab || gear == GrenadeRegistration.CatalogGear)
            return true;

        if (gear.Info != null && gear.Info.APIName == ThermitePlugin.GearApiName)
            return true;

        if (gear.Info != null && gear.Info.ID == ThermitePlugin.GearId)
            return true;
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            ThermitePlugin.Logger?.LogError(
                "[Thermite] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
    ...
    private static void OnCloseCallbackPrefix()
    {
        if (GrenadeRegistration.BaseAllGearIndex < 0 && Global.Instance?.AllGear != null)
        {
            ...
            {
                if (Global.Instance.AllGear[g] is IncendiaryGrenade &&
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            ThermitePlugin.Logger?.LogError(
                "[Thermite] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
