...
/// <summary>
/// Fixes equipping runtime-cloned Cavity Scrapworks.
///
...
///
/// Strategy:
///  1. Prefix remaps allGearIndex to the vanilla PlateLauncher base gun index.
///  2. Postfix stamps our GearInfo + CavityScrapworksBehaviour onto the live instance.
///  3. Null-guard GearSelectionWindow so a failed equip cannot soft-lock UI.
///
/// Live gun MUST remain a PlateLauncher so PlateBullet.OnInitialized cast succeeds.
/// </summary>
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[CavityScrapworks] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[CavityScrapworks] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[CavityScrapworks] Cannot remap spawn — PlateLauncher AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        SparrohPlugin.Logger?.LogInfo(
            $"[CavityScrapworks] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        if (baseIndex >= 0 && baseIndex < Global.Instance.AllGear.Length)
        {
            ...
            if (at != null && !IsOurCatalogGear(at) && at is PlateLauncher)
                return baseIndex;
        }
        ...
        if (WeaponRegistration.BaseGunPrefab != null)
        {
            ...
            if (idx >= 0 && !IsOurCatalogGear(Global.Instance.AllGear[idx]))
            {
        ...
        // Prefer a real PlateLauncher that is not our catalog entry.
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
            if (Global.Instance.AllGear[i] is PlateLauncher &&
                !IsOurCatalogGear(Global.Instance.AllGear[i]))
            {
                WeaponRegistration.SetBaseAllGearIndex(i);
                return i;
        ...
        // Last resort: any non-catalog primary (will break plate cast — log loudly).
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
                ...
                !IsOurCatalogGear(Global.Instance.AllGear[i]))
            {
                SparrohPlugin.Logger?.LogWarning(
                    $"[CavityScrapworks] No PlateLauncher base found — falling back to {gun.GetType().Name}.");
                WeaponRegistration.SetBaseAllGearIndex(i);
                return i;
            }
        }
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[CavityScrapworks] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[CavityScrapworks] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[CavityScrapworks] Catalog gear missing during stamp.");
            return;
        }
    ...
    /// <summary>
    /// After NGO spawns the vanilla PlateLauncher, rebind the live instance to our catalog identity
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            CavityScrapworksBehaviour behaviour = live.gameObject.GetComponent<CavityScrapworksBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<CavityScrapworksBehaviour>();

            CavityScrapworksBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<CavityScrapworksBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[CavityScrapworks] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        // Ensure plate stats stick even if ApplyUpgrades path is partial.
        if (live is Gun liveGun && catalog is Gun catalogGun)
        {
            // Catalog already has CsBalance GunData; re-apply if damage still looks wrong.
            if (Mathf.Abs(liveGun.GunData.damage - CsBalance.Damage) > 0.5f &&
                Mathf.Abs(catalogGun.GunData.damage - CsBalance.Damage) < 0.5f)
            {
                WeaponRegistration.ApplyPlateStats(liveGun);
            }

            if (CavityScrapworksBehaviour.TryGet(liveGun, out var csLive))
                csLive.OnUpgradesApplied(liveGun);
        }

        PersistEquippedCatalogId(catalog, slot);

        SparrohPlugin.Logger?.LogInfo(
            $"[CavityScrapworks] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
            $"(Info={live.Info?.APIName}, Prefab={(live.Prefab as Component)?.name}, " +
            $"type={live.GetType().Name}).");
    }
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[CavityScrapworks] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[CavityScrapworks] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[CavityScrapworks] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[CavityScrapworks] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[CavityScrapworks] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            int baseIndex = WeaponRegistration.BaseAllGearIndex;
            if (baseIndex < 0 ||
                baseIndex >= Global.Instance.AllGear.Length ||
                SpawnGearHooks.IsOurCatalogGear(Global.Instance.AllGear[baseIndex]) ||
                Global.Instance.AllGear[baseIndex] is not PlateLauncher)
            {
                baseIndex = -1;
                for (int g = 0; g < Global.Instance.AllGear.Length; g++)
                {
                    if (Global.Instance.AllGear[g] is PlateLauncher &&
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[CavityScrapworks] OnClose: no PlateLauncher base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[CavityScrapworks] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[CavityScrapworks] OnClose stamp: {ex.Message}");
            }
        }
