...
/// <summary>
/// Final Judgement — SAXON man-portable strategic charge heavy for Mycopunk.
///
/// Phase 0: registration / HeavyNuke clone / persistence / equip stamp.
/// Phase 1: baseline 8s authorize → mag-1 rocket → classic ImpactSphere (no upgrades).
/// </summary>
[BepInPlugin(PluginGUID, PluginName, PluginVersion)]
[MycoMod(null, ModFlags.IsSandbox)]
public class SparrohPlugin : BaseUnityPlugin
{
    public const string PluginGUID = "sparroh.finaljudgement";
    public const string PluginName = "FinalJudgement";
    public const string PluginVersion = "0.1.0";

    /// <summary>Stable GearInfo.ID — 93100 band.</summary>
    public const int GearId = 93100;

    /// <summary>GearInfo.APIName — FindGear / AllGear / identity gates.</summary>
    public const string GearApiName = "final_judgement";

    public const string GearDisplayName = "Final Judgement";
    public const string GearDescription =
        "SAXON FJ-1 Final Judgement — Field strategic charge system. " +
        "Full authorization cycle: eight seconds. Magazine 1. " +
        "Baseline: rocket with classic explosive sphere. " +
        "Hammer / Warhead / Retribution packages sold separately.";

    /// <summary>Vanilla gun to clone (The Last Argument — charge rocket + sphere boom).</summary>
    public const string BaseTypeName = "HeavyNuke";
    ...
    private void Awake()
    {
        ...
        FinalJudgementCombatHooks.Apply(_harmony);
    ...
    internal void TryRegisterGear(string reason)
    {
        ...
        {
            Logger.LogDebug($"[FinalJudgement] Global.AllGear not ready yet ({reason}).");
            return;
        }
        ...
        try
        {
            ...
            Logger.LogInfo(
                $"[FinalJudgement] Registered gear '{GearDisplayName}' " +
                $"(api={GearApiName}, id={GearId}) via {reason}.");
        }
        catch (Exception ex)
        {
            Logger.LogError($"[FinalJudgement] Gear registration failed: {ex}");
        }
    }
    ...
    /// <summary>True when this gear is Final Judgement (not vanilla HeavyNuke).</summary>
...
/// <summary>
/// Keep Final Judgement alive across save load.
...
internal static class PlayerDataPersistenceHooks
{
    ...
    private static void Postfix()
    {
        try
        {
            ...
            if (gear == null)
            {
                SparrohPlugin.Logger?.LogWarning("[FinalJudgement] Persistence: gear missing after OnAwake.");
                return;
            }
        ...
        {
            SparrohPlugin.Logger?.LogError($"[FinalJudgement] Persistence postfix failed: {ex}");
        }
    }
    ...
    private static void TryEnsureSavedLoadoutPointsAtCatalog(IUpgradable gear)
    {
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[FinalJudgement] Save loadout references catalog id={id} " +
