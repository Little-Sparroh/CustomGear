...
///
/// Strategy:
///  1. Prefix with <c>ref int allGearIndex</c> so Harmony writes the remapped HeavyNuke index.
///  2. Postfix: stamp GearInfo + FinalJudgementBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[FinalJudgement] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[FinalJudgement] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[FinalJudgement] Cannot remap spawn — base HeavyNuke AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        SparrohPlugin.Logger?.LogInfo(
            $"[FinalJudgement] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        // Prefer HeavyNuke specifically.
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
            if (Global.Instance.AllGear[i] is HeavyNuke &&
                !IsOurCatalogGear(Global.Instance.AllGear[i]))
            {
                WeaponRegistration.SetBaseAllGearIndex(i);
                return i;
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[FinalJudgement] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[FinalJudgement] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[FinalJudgement] Catalog gear missing during stamp.");
            return;
        }
    ...
    /// <summary>
    /// After NGO spawns vanilla HeavyNuke, rebind the live instance to our catalog identity
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            FinalJudgementBehaviour behaviour = live.gameObject.GetComponent<FinalJudgementBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<FinalJudgementBehaviour>();

            FinalJudgementBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<FinalJudgementBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[FinalJudgement] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        if (live is Gun liveGun)
        {
            // Always re-apply full baseline after stamp — ApplyUpgrades can restore HeavyNuke stats.
            WeaponRegistration.ApplyFinalJudgementStats(liveGun, SparrohPlugin.Logger);

            liveGun.IsAimEnabled = FjBalance.IsAimEnabled;
            liveGun.GunData.automatic = FjBalance.Automatic;
            liveGun.GunData.hitForce = FjBalance.HitForce;
            liveGun.GunData.chargeData.duration = FjBalance.ChargeDuration;
            liveGun.GunData.chargeData.fireWhenFullyCharged = FjBalance.ChargeFireWhenFullyCharged;
            liveGun.GunData.magazineSize = FjBalance.MagazineSize;

            if (FinalJudgementBehaviour.TryGet(liveGun, out var fj))
                fj.OnUpgradesApplied(liveGun);

            SparrohPlugin.Logger?.LogInfo(
                $"[FinalJudgement] Live stats dmg={liveGun.GunData.damage} " +
                $"force={liveGun.GunData.hitForce} charge={liveGun.GunData.chargeData.duration}s " +
                $"mag={liveGun.GunData.magazineSize} api={liveGun.Info?.APIName} id={liveGun.Info?.ID}");
        }

        PersistEquippedCatalogId(catalog, slot);

        SparrohPlugin.Logger?.LogInfo(
            $"[FinalJudgement] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[FinalJudgement] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[FinalJudgement] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[FinalJudgement] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[FinalJudgement] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[FinalJudgement] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            if (baseIndex < 0)
            {
                ...
                {
                    if (Global.Instance.AllGear[g] is HeavyNuke &&
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[FinalJudgement] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[FinalJudgement] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[FinalJudgement] OnClose stamp: {ex.Message}");
            }
        }
