...
///
/// Strategy:
///  1. Prefix with <c>ref int allGearIndex</c> so Harmony writes the remapped HeavyLaser index.
///  2. Postfix: stamp GearInfo + ThermalSolsticeBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[ThermalSolstice] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[ThermalSolstice] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[ThermalSolstice] Cannot remap spawn — base HeavyLaser AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        SparrohPlugin.Logger?.LogInfo(
            $"[ThermalSolstice] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        // Prefer vanilla HeavyLaser that is not our catalog entry.
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
            if (Global.Instance.AllGear[i] is HeavyLaser &&
                !IsOurCatalogGear(Global.Instance.AllGear[i]))
            {
                WeaponRegistration.SetBaseAllGearIndex(i);
                return i;
        ...
        // Last resort: first Heavy gun that is not our catalog entry.
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[ThermalSolstice] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[ThermalSolstice] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[ThermalSolstice] Catalog gear missing during stamp.");
            return;
        }
    ...
    /// <summary>
    /// After NGO spawns vanilla HeavyLaser, rebind the live instance to our catalog identity
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            ThermalSolsticeBehaviour behaviour = live.gameObject.GetComponent<ThermalSolsticeBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<ThermalSolsticeBehaviour>();

            ThermalSolsticeBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<ThermalSolsticeBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[ThermalSolstice] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        if (live is Gun liveGun)
        {
            // Always re-apply full baseline after stamp — ApplyUpgrades can restore HeavyLaser stats.
            WeaponRegistration.ApplyThermalSolsticeStats(liveGun, SparrohPlugin.Logger);

            liveGun.IsAimEnabled = TsBalance.IsAimEnabled;
            liveGun.GunData.automatic = TsBalance.Automatic;
            liveGun.GunData.damageEffect = TsBalance.DamageEffect;
            liveGun.GunData.damageEffectAmount = TsBalance.DamageEffectAmount;
            liveGun.GunData.magazineSize = TsBalance.MagazineSize;
            liveGun.GunData.fireInterval = TsBalance.FireInterval;

            if (ThermalSolsticeBehaviour.TryGet(liveGun, out var ts))
                ts.OnUpgradesApplied(liveGun);

            SparrohPlugin.Logger?.LogInfo(
                $"[ThermalSolstice] Live stats dmg={liveGun.GunData.damage} " +
                $"fire={liveGun.GunData.damageEffect}/{liveGun.GunData.damageEffectAmount} " +
                $"interval={liveGun.GunData.fireInterval} mag={liveGun.GunData.magazineSize} " +
                $"api={liveGun.Info?.APIName} id={liveGun.Info?.ID}");
        }

        PersistEquippedCatalogId(catalog, slot);

        SparrohPlugin.Logger?.LogInfo(
            $"[ThermalSolstice] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
        ...
        try
        {
            ...
            switch (slot)
            {
                ...
                default:
                    ...
                    {
                        if (g.GearType == GearType.Primary || g.GearType == GearType.Heavy)
                            PlayerData.Instance.weapon1ID = id;
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[ThermalSolstice] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[ThermalSolstice] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[ThermalSolstice] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[ThermalSolstice] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[ThermalSolstice] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            if (baseIndex < 0)
            {
                ...
                {
                    if (Global.Instance.AllGear[g] is HeavyLaser &&
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[ThermalSolstice] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[ThermalSolstice] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[ThermalSolstice] OnClose stamp: {ex.Message}");
            }
        }
