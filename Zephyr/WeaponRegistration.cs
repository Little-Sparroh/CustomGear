...
/// <summary>
/// Creates and registers Zephyr by cloning vanilla TheCarver and applying balance stats.
/// </summary>
public static class WeaponRegistration
{
        ...
        out IUpgradable registeredGear)
    {
        ...
        if (existing != null)
        {
            ...
            if (existing is Gun existingGun)
                ApplyZephyrStats(existingGun, log);
        ...
        ZephyrWeaponBehaviour behaviour = clone.GetComponent<ZephyrWeaponBehaviour>();
        if (behaviour == null)
            behaviour = clone.AddComponent<ZephyrWeaponBehaviour>();
        behaviour.InitializeAsPrefab(description);

        ApplyZephyrStats(cloneGun, log);
    ...
    /// <summary>
    /// Rewrites GunData / aim / CarverData from <see cref="ZephyrBalance"/>.
    /// Call on catalog create and live spawn stamp.
    /// </summary>
    public static void ApplyZephyrStats(Gun gun, ManualLogSource log = null)
    {
        if (gun == null)
            return;
        ...
        g.damage = ZephyrBalance.Damage;
        g.damageEffect = ZephyrBalance.DamageEffect;
        g.damageEffectAmount = ZephyrBalance.DamageEffectAmount;
        g.fireInterval = ZephyrBalance.FireInterval;
        g.fireAnimationSpeedMultiplier = ZephyrBalance.FireAnimationSpeedMultiplier;
        g.automatic = ZephyrBalance.Automatic;
        g.bulletsPerShot = ZephyrBalance.BulletsPerShot;
        g.burstSize = ZephyrBalance.BurstSize;
        g.burstFireInterval = ZephyrBalance.BurstFireInterval;
        g.useAmmoOnFire = ZephyrBalance.UseAmmoOnFire;
        g.doesEachBulletInShotRemoveAmmo = ZephyrBalance.DoesEachBulletInShotRemoveAmmo;
        g.doesEachBulletInShotTriggerEffects = ZephyrBalance.DoesEachBulletInShotTriggerEffects;
        g.hitForce = ZephyrBalance.HitForce;
        g.hitVFXSize = ZephyrBalance.HitVfxSize;

        g.magazineSize = ZephyrBalance.MagazineSize;
        g.hasLimitedAmmo = ZephyrBalance.HasLimitedAmmo;
        g.ammoCapacity = ZephyrBalance.AmmoCapacity;
        g.ammoCollectMultiplier = ZephyrBalance.AmmoCollectMultiplier;
        g.storedAmmoCollectMultiplier = ZephyrBalance.StoredAmmoCollectMultiplier;
        g.ammoGenerationEfficiency = ZephyrBalance.AmmoGenerationEfficiency;
        g.useAmmoWhileFiringInterval = ZephyrBalance.UseAmmoWhileFiringInterval;
        g.refillAmmoOnReload = ZephyrBalance.RefillAmmoOnReload;
        g.reloadDuration = ZephyrBalance.ReloadDuration;
        g.autoReloadWhenEmpty = ZephyrBalance.AutoReloadWhenEmpty;

        g.bulletSpeed = ZephyrBalance.BulletSpeed;
        g.bulletGravity = ZephyrBalance.BulletGravity;
        g.maxBounces = ZephyrBalance.MaxBounces;
        g.bulletMagnetismSurface = ZephyrBalance.BulletMagnetismSurface;
        g.bulletMagnetismTarget = ZephyrBalance.BulletMagnetismTarget;
        g.bulletShakeTranslation = ZephyrBalance.BulletShakeTranslation;
        g.bulletShakeRotation = ZephyrBalance.BulletShakeRotation;

        g.rangeData.maxDamageRange = ZephyrBalance.MaxDamageRange;
        g.rangeData.falloffStartDistance = ZephyrBalance.FalloffStartDistance;
        g.rangeData.falloffEndDistance = ZephyrBalance.FalloffEndDistance;
        g.rangeData.maxFalloffDamageMultiplier = ZephyrBalance.MaxFalloffDamageMultiplier;

        g.spreadData.spreadType = ZephyrBalance.SpreadType;
        g.spreadData.spreadSize = ZephyrBalance.SpreadSize;
        g.firstShotSpreadMultiplier = ZephyrBalance.FirstShotSpreadMultiplier;

        g.recoilData.recoilX = ZephyrBalance.RecoilX;
        g.recoilData.recoilY = ZephyrBalance.RecoilY;
        g.recoilData.recoilZ = ZephyrBalance.RecoilZ;
        g.recoilData.maxRecoilZ = ZephyrBalance.MaxRecoilZ;
        g.recoilData.translateZ = ZephyrBalance.TranslateZ;
        g.recoilData.maxTranslateZ = ZephyrBalance.MaxTranslateZ;
        g.recoilData.aimTranslateMultiplier = ZephyrBalance.AimTranslateMultiplier;
        g.recoilData.recoilSpeed = ZephyrBalance.RecoilSpeed;
        g.recoilData.recoilRecoverySpeed = ZephyrBalance.RecoilRecoverySpeed;
        g.recoilData.translateSpeed = ZephyrBalance.TranslateSpeed;
        g.recoilData.translateRecoverySpeed = ZephyrBalance.TranslateRecoverySpeed;
        g.recoilData.recoilTargetDecaySpeed = ZephyrBalance.RecoilTargetDecaySpeed;
        g.recoilData.aimRecoilMultiplier = ZephyrBalance.AimRecoilMultiplier;

        g.chargeData.duration = ZephyrBalance.ChargeDuration;
        g.chargeData.coolDownSpeed = ZephyrBalance.ChargeCoolDownSpeed;
        g.chargeData.fireWhenFullyCharged = ZephyrBalance.ChargeFireWhenFullyCharged;
        g.chargeData.fireOnRelease = ZephyrBalance.ChargeFireOnRelease;
        g.chargeData.canFireWhileCharging = ZephyrBalance.ChargeCanFireWhileCharging;
        g.chargeData.time = 0f;

        g.fireConstraints.canFireWhileSprinting = ZephyrBalance.CanFireWhileSprinting;
        g.fireConstraints.canFireWhileSliding = ZephyrBalance.CanFireWhileSliding;
        g.fireConstraints.canAimWhileSliding = ZephyrBalance.CanAimWhileSliding;
        g.fireConstraints.canAimWhileReloading = ZephyrBalance.CanAimWhileReloading;
        g.fireConstraints.canReloadWhileSprinting = ZephyrBalance.CanReloadWhileSprinting;

        // Free Aim/RMB for later path overrides.
        gun.IsAimEnabled = ZephyrBalance.IsAimEnabled;
        gun.AimFOV = ZephyrBalance.AimFov;
        TrySetAimTransitionDuration(gun, ZephyrBalance.AimTransitionDuration);

        if (gun is TheCarver carver)
        {
            ref TheCarver.CarverData cd = ref carver.Data;
            cd.damageArea = ZephyrBalance.DamageArea;

            // Suppress vanilla Carver blood/rage/shackle baseline — Zephyr owns blast only.
            cd.maxBlood = 0f;
            cd.bloodOnKill = 0f;
            cd.bloodDamage = 0f;
