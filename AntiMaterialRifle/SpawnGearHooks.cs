...
internal static class SpawnGearHooks
{
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            AntiMaterialRifleBehaviour behaviour = live.gameObject.GetComponent<AntiMaterialRifleBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<AntiMaterialRifleBehaviour>();

            AntiMaterialRifleBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<AntiMaterialRifleBehaviour>();
        ...
        if (live is Gun liveGun && catalog is Gun catalogGun)
        {
            ...
            if (AntiMaterialRifleBehaviour.TryGet(liveGun, out var sniperLive))
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        {
            SparrohPlugin.Logger?.LogWarning($"[BallisticSniper] PersistEquippedCatalogId: {ex.Message}");
        }
