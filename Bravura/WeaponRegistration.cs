...
///  5. Attaches <see cref="BravuraBehaviour"/> for custom data / combat hooks
///     Clones FastReloadShotgun (Lead Flinger) by default.
...
public static class WeaponRegistration
{
        ...
        out IUpgradable registeredGear)
    {
        ...
        if (existing != null)
        {
            ...
            if (existing is Gun existingGun)
                ApplyBravuraStats(existingGun, log);
        ...
        // Custom behaviour host — Style / verbs.
        BravuraBehaviour behaviour = clone.GetComponent<BravuraBehaviour>();
        if (behaviour == null)
            behaviour = clone.AddComponent<BravuraBehaviour>();
        behaviour.InitializeAsPrefab(description);

        ApplyBravuraStats(cloneGun, log);
    ...
    /// <summary>
    /// Rewrites GunData / aim fields from <see cref="BravuraBalance"/>.
    /// Also strips vanilla Lead Flinger kill→reload on FastReloadShotgun clones.
    /// </summary>
    public static void ApplyBravuraStats(Gun gun, ManualLogSource log = null)
    {
        if (gun == null)
            return;
        ...
        g.damage = BravuraBalance.Damage;
        g.damageEffect = BravuraBalance.DamageEffect;
        g.damageEffectAmount = BravuraBalance.DamageEffectAmount;
        g.fireInterval = BravuraBalance.FireInterval;
        g.fireAnimationSpeedMultiplier = BravuraBalance.FireAnimationSpeedMultiplier;
        g.automatic = BravuraBalance.Automatic;
        g.bulletsPerShot = BravuraBalance.BulletsPerShot;
        g.burstSize = BravuraBalance.BurstSize;
        g.burstFireInterval = BravuraBalance.BurstFireInterval;
        g.useAmmoOnFire = BravuraBalance.UseAmmoOnFire;
        g.doesEachBulletInShotRemoveAmmo = BravuraBalance.DoesEachBulletInShotRemoveAmmo;
        g.doesEachBulletInShotTriggerEffects = BravuraBalance.DoesEachBulletInShotTriggerEffects;
        g.hitForce = BravuraBalance.HitForce;
        g.hitVFXSize = BravuraBalance.HitVfxSize;

        g.magazineSize = BravuraBalance.MagazineSize;
        g.hasLimitedAmmo = BravuraBalance.HasLimitedAmmo;
        g.ammoCapacity = BravuraBalance.AmmoCapacity;
        g.ammoCollectMultiplier = BravuraBalance.AmmoCollectMultiplier;
        g.storedAmmoCollectMultiplier = BravuraBalance.StoredAmmoCollectMultiplier;
        g.ammoGenerationEfficiency = BravuraBalance.AmmoGenerationEfficiency;
        g.useAmmoWhileFiringInterval = BravuraBalance.UseAmmoWhileFiringInterval;
        g.refillAmmoOnReload = BravuraBalance.RefillAmmoOnReload;
        g.reloadDuration = BravuraBalance.ReloadDuration;
        g.autoReloadWhenEmpty = BravuraBalance.AutoReloadWhenEmpty;

        g.bulletSpeed = BravuraBalance.BulletSpeed;
        g.bulletGravity = BravuraBalance.BulletGravity;
        g.maxBounces = BravuraBalance.MaxBounces;
        g.bulletMagnetismSurface = BravuraBalance.BulletMagnetismSurface;
        g.bulletMagnetismTarget = BravuraBalance.BulletMagnetismTarget;
        g.bulletShakeTranslation = BravuraBalance.BulletShakeTranslation;
        g.bulletShakeRotation = BravuraBalance.BulletShakeRotation;

        g.rangeData.falloffStartDistance = BravuraBalance.FalloffStartDistance;
        g.rangeData.falloffEndDistance = BravuraBalance.FalloffEndDistance;
        g.rangeData.maxDamageRange = BravuraBalance.MaxDamageRange;
        g.rangeData.maxFalloffDamageMultiplier = BravuraBalance.MaxFalloffDamageMultiplier;

        g.spreadData.spreadType = BravuraBalance.SpreadType;
        g.spreadData.spreadSize = BravuraBalance.SpreadSize;
        g.firstShotSpreadMultiplier = BravuraBalance.FirstShotSpreadMultiplier;

        g.recoilData.recoilX = BravuraBalance.RecoilX;
        g.recoilData.recoilY = BravuraBalance.RecoilY;
        g.recoilData.recoilZ = BravuraBalance.RecoilZ;
        g.recoilData.maxRecoilZ = BravuraBalance.MaxRecoilZ;
        g.recoilData.translateZ = BravuraBalance.TranslateZ;
        g.recoilData.maxTranslateZ = BravuraBalance.MaxTranslateZ;
        g.recoilData.aimTranslateMultiplier = BravuraBalance.AimTranslateMultiplier;
        g.recoilData.recoilSpeed = BravuraBalance.RecoilSpeed;
        g.recoilData.recoilRecoverySpeed = BravuraBalance.RecoilRecoverySpeed;
        g.recoilData.translateSpeed = BravuraBalance.TranslateSpeed;
        g.recoilData.translateRecoverySpeed = BravuraBalance.TranslateRecoverySpeed;
        g.recoilData.recoilTargetDecaySpeed = BravuraBalance.RecoilTargetDecaySpeed;
        g.recoilData.aimRecoilMultiplier = BravuraBalance.AimRecoilMultiplier;

        g.chargeData.duration = BravuraBalance.ChargeDuration;
        g.chargeData.coolDownSpeed = BravuraBalance.ChargeCoolDownSpeed;
        g.chargeData.fireWhenFullyCharged = BravuraBalance.ChargeFireWhenFullyCharged;
        g.chargeData.fireOnRelease = BravuraBalance.ChargeFireOnRelease;
        g.chargeData.canFireWhileCharging = BravuraBalance.ChargeCanFireWhileCharging;
        g.chargeData.time = 0f;

        g.fireConstraints.canFireWhileSprinting = BravuraBalance.CanFireWhileSprinting;
        g.fireConstraints.canFireWhileSliding = BravuraBalance.CanFireWhileSliding;
        g.fireConstraints.canAimWhileSliding = BravuraBalance.CanAimWhileSliding;
        g.fireConstraints.canAimWhileReloading = BravuraBalance.CanAimWhileReloading;
        g.fireConstraints.canReloadWhileSprinting = BravuraBalance.CanReloadWhileSprinting;

        gun.IsAimEnabled = BravuraBalance.IsAimEnabled;
        gun.AimFOV = BravuraBalance.AimFov;
        TrySetAimTransitionDuration(gun, BravuraBalance.AimTransitionDuration);

        BravuraBehaviour.StripLeadFlingerVanilla(gun);
    }
