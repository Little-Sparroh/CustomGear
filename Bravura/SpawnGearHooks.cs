...
///  2. Postfix: stamp our GearInfo + BravuraBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[Bravura] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[Bravura] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[Bravura] Cannot remap spawn — base gun AllGear index unknown. " +
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[Bravura] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[Bravura] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[Bravura] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[Bravura] Catalog gear missing during stamp.");
            return;
        }
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            BravuraBehaviour behaviour = live.gameObject.GetComponent<BravuraBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<BravuraBehaviour>();

            BravuraBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<BravuraBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[Bravura] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        // 4) Re-assert balance + strip LF kill-reload after ApplyUpgrades.
        if (live is Gun liveGun)
        {
            if (catalog is Gun catalogGun && NeedsBravuraStatRepair(liveGun, catalogGun))
                WeaponRegistration.ApplyBravuraStats(liveGun, SparrohPlugin.Logger);

            liveGun.IsAimEnabled = BravuraBalance.IsAimEnabled;
            liveGun.GunData.automatic = BravuraBalance.Automatic;

            if (BravuraBehaviour.TryGet(liveGun, out var b))
                b.OnUpgradesApplied(liveGun);

            BravuraBehaviour.StripLeadFlingerVanilla(liveGun);
        }
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[Bravura] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
            $"(Info={live.Info?.APIName}, Prefab={(live.Prefab as Component)?.name}).");
    }

    private static bool NeedsBravuraStatRepair(Gun live, Gun catalog)
    {
        if (live == null || catalog == null)
            return false;

        if (catalog.GunData.magazineSize == BravuraBalance.MagazineSize &&
            live.GunData.magazineSize != BravuraBalance.MagazineSize)
            return true;
        ...
        if (live.IsAimEnabled != BravuraBalance.IsAimEnabled)
            return true;
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[Bravura] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[Bravura] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[Bravura] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[Bravura] Safe OnCloseCallback failed: {ex}");
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[Bravura] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[Bravura] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[Bravura] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[Bravura] OnClose stamp: {ex.Message}");
