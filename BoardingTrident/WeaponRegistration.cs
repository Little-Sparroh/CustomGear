...
/// <summary>
/// Creates and registers the Boarding Trident at runtime by cloning WideGun (Trident S2)
/// and rewriting GunData / tridentData into the flipped-axis boarding profile.
/// </summary>
public static class WeaponRegistration
{
        ...
        out IUpgradable registeredGear)
    {
        ...
        BoardingTridentBehaviour behaviour = clone.GetComponent<BoardingTridentBehaviour>();
        if (behaviour == null)
            behaviour = clone.AddComponent<BoardingTridentBehaviour>();
        behaviour.InitializeAsPrefab(description);

        ApplyBoardingTridentStats(cloneGun, log);
        BoardingTridentBehaviour.ApplyWideGunBaseline(cloneGun);
        behaviour.CaptureStanceCaches(cloneGun);
    ...
    public static void ApplyBoardingTridentStats(Gun gun, ManualLogSource log = null)
    {
        if (gun == null)
            return;
        ...
        g.damage = BoardingTridentBalance.Damage;
        g.damageEffect = BoardingTridentBalance.DamageEffect;
        g.damageEffectAmount = BoardingTridentBalance.DamageEffectAmount;
        g.fireInterval = BoardingTridentBalance.FireInterval;
        g.fireAnimationSpeedMultiplier = Mathf.Max(
            g.fireAnimationSpeedMultiplier, BoardingTridentBalance.FireAnimationSpeedMultiplier);
        g.automatic = BoardingTridentBalance.Automatic;
        g.bulletsPerShot = BoardingTridentBalance.BulletsPerShot;
        g.burstSize = BoardingTridentBalance.BurstSize;
        g.burstFireInterval = BoardingTridentBalance.BurstFireInterval;
        g.useAmmoOnFire = BoardingTridentBalance.UseAmmoOnFire;
        g.doesEachBulletInShotRemoveAmmo = BoardingTridentBalance.DoesEachBulletInShotRemoveAmmo;
        g.doesEachBulletInShotTriggerEffects = BoardingTridentBalance.DoesEachBulletInShotTriggerEffects;

        g.magazineSize = BoardingTridentBalance.MagazineSize;
        g.hasLimitedAmmo = BoardingTridentBalance.HasLimitedAmmo;
        g.ammoCapacity = BoardingTridentBalance.AmmoCapacity;
        g.ammoCollectMultiplier = Mathf.Max(
            g.ammoCollectMultiplier, BoardingTridentBalance.AmmoCollectMultiplier);
        g.storedAmmoCollectMultiplier = Mathf.Max(
            g.storedAmmoCollectMultiplier, BoardingTridentBalance.StoredAmmoCollectMultiplier);
        g.ammoGenerationEfficiency = BoardingTridentBalance.AmmoGenerationEfficiency;
        g.useAmmoWhileFiringInterval = BoardingTridentBalance.UseAmmoWhileFiringInterval;
        g.refillAmmoOnReload = BoardingTridentBalance.RefillAmmoOnReload;
        g.reloadDuration = BoardingTridentBalance.ReloadDuration;
        g.autoReloadWhenEmpty = BoardingTridentBalance.AutoReloadWhenEmpty;

        g.bulletSpeed = BoardingTridentBalance.BulletSpeed;
        g.bulletGravity = BoardingTridentBalance.BulletGravity;
        g.maxBounces = BoardingTridentBalance.MaxBounces;
        g.bulletMagnetismSurface = BoardingTridentBalance.BulletMagnetismSurface;
        g.bulletMagnetismTarget = BoardingTridentBalance.BulletMagnetismTarget;

        g.rangeData.falloffStartDistance = BoardingTridentBalance.FalloffStartDistance;
        g.rangeData.falloffEndDistance = BoardingTridentBalance.FalloffEndDistance;
        g.rangeData.maxDamageRange = BoardingTridentBalance.MaxDamageRange;
        g.rangeData.maxFalloffDamageMultiplier = BoardingTridentBalance.MaxFalloffDamageMultiplier;

        g.hitForce = BoardingTridentBalance.HitForce;
        g.hitVFXSize = BoardingTridentBalance.HitVfxSize;

        // Hip spread = horizontal rake (flipped from vanilla vertical)
        g.spreadData.spreadType = BoardingTridentBalance.SpreadType;
        g.spreadData.spreadSize = BoardingTridentBalance.HipSpreadSize;
        g.firstShotSpreadMultiplier = BoardingTridentBalance.FirstShotSpreadMultiplier;

        g.recoilData.recoilX = BoardingTridentBalance.RecoilX;
        g.recoilData.recoilY = BoardingTridentBalance.RecoilY;
        g.recoilData.recoilZ = BoardingTridentBalance.RecoilZ;
        g.recoilData.maxRecoilZ = BoardingTridentBalance.MaxRecoilZ;
        g.recoilData.translateZ = BoardingTridentBalance.TranslateZ;
        g.recoilData.maxTranslateZ = BoardingTridentBalance.MaxTranslateZ;
        g.recoilData.aimTranslateMultiplier = BoardingTridentBalance.AimTranslateMultiplier;
        g.recoilData.recoilSpeed = Mathf.Max(g.recoilData.recoilSpeed, BoardingTridentBalance.RecoilSpeed);
        g.recoilData.recoilRecoverySpeed = BoardingTridentBalance.RecoilRecoverySpeed;
        g.recoilData.translateSpeed = Mathf.Max(g.recoilData.translateSpeed, BoardingTridentBalance.TranslateSpeed);
        g.recoilData.translateRecoverySpeed = BoardingTridentBalance.TranslateRecoverySpeed;
        g.recoilData.recoilTargetDecaySpeed = BoardingTridentBalance.RecoilTargetDecaySpeed;
        g.recoilData.aimRecoilMultiplier = BoardingTridentBalance.AimRecoilMultiplier;

        g.bulletShakeTranslation = BoardingTridentBalance.BulletShakeTranslation;
        g.bulletShakeRotation = BoardingTridentBalance.BulletShakeRotation;

        g.chargeData.duration = BoardingTridentBalance.ChargeDuration;
        g.chargeData.coolDownSpeed = BoardingTridentBalance.ChargeCoolDownSpeed;
        g.chargeData.fireWhenFullyCharged = BoardingTridentBalance.ChargeFireWhenFullyCharged;
        g.chargeData.fireOnRelease = BoardingTridentBalance.ChargeFireOnRelease;
        g.chargeData.canFireWhileCharging = BoardingTridentBalance.ChargeCanFireWhileCharging;
        g.chargeData.time = 0f;

        g.fireConstraints.canFireWhileSprinting = BoardingTridentBalance.CanFireWhileSprinting;
        g.fireConstraints.canFireWhileSliding = BoardingTridentBalance.CanFireWhileSliding;
        g.fireConstraints.canAimWhileSliding = BoardingTridentBalance.CanAimWhileSliding;
        g.fireConstraints.canAimWhileReloading = BoardingTridentBalance.CanAimWhileReloading;
        g.fireConstraints.canReloadWhileSprinting = BoardingTridentBalance.CanReloadWhileSprinting;

        gun.IsAimEnabled = BoardingTridentBalance.IsAimEnabled;
        gun.AimFOV = BoardingTridentBalance.AimFov;
        TrySetAimTransitionDuration(gun, BoardingTridentBalance.AimTransitionDuration);

        // Clear vanilla WideGun upgrade flags on catalog baseline.
        try
        {
        ...
        BoardingTridentBehaviour.ApplyWideGunBaseline(gun);

        //log?.LogInfo(
            //$"[WeaponRegistration] Applied boarding trident stats: dmg={g.damage}, " +
            //$"rpm≈{60f / g.fireInterval:0}, bps={g.bulletsPerShot}, " +
            //$"mag={g.magazineSize}, reserve={g.ammoCapacity}, " +
            //$"reload={g.reloadDuration}s, hipSpread={g.spreadData.spreadSize}, " +
        ...
        ManualLogSource log)
    {
        ...
        // Isolate upgrade pool from WideGun.
...
public static class ModelImportHooks
{
    ...
    {
        log?.LogDebug("[ModelImportHooks] Placeholder only — using vanilla WideGun (Trident) visuals.");
    }
