...
///
/// Strategy:
///  1. Prefix remaps allGearIndex to the vanilla WideGun base gun index.
///  2. Postfix stamps our GearInfo + BoardingTridentBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[BoardingTrident] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[BoardingTrident] Cannot remap spawn — base gun AllGear index unknown. " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        // Prefer WideGun specifically.
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
            if (Global.Instance.AllGear[i] is WideGun &&
                !IsOurCatalogGear(Global.Instance.AllGear[i]))
            {
                WeaponRegistration.SetBaseAllGearIndex(i);
                return i;
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[BoardingTrident] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[BoardingTrident] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[BoardingTrident] Catalog gear missing during stamp.");
            return;
        }
    ...
    /// <summary>
    /// After NGO spawns the vanilla WideGun, rebind the live instance to our catalog identity
    /// and re-run IGear.ApplyUpgrades so GunData restores from our boarding trident catalog baseline.
    /// </summary>
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            BoardingTridentBehaviour behaviour = live.gameObject.GetComponent<BoardingTridentBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<BoardingTridentBehaviour>();

            BoardingTridentBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<BoardingTridentBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[BoardingTrident] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        if (live is Gun liveGun)
        {
            // Always re-apply boarding stats on live WideGun spawn.
            // Catalog-only AimFOV=0 is lost because NGO instantiates vanilla prefab.
            WeaponRegistration.ApplyBoardingTridentStats(liveGun);
            liveGun.AimFOV = BoardingTridentBalance.AimFov;

            if (BoardingTridentBehaviour.TryGet(liveGun, out var btLive))
            {
                btLive.OnUpgradesApplied(liveGun);
                btLive.RotationT = 0f;
            }

            // HUD may already be enabled from vanilla Setup — force boarding hip crosshair.
            try
            {
                BoardingTridentHudHooks.RefreshHudForGun(liveGun);
            }
            catch (Exception ex)
            {
                SparrohPlugin.Logger?.LogDebug($"[BoardingTrident] HUD refresh after stamp: {ex.Message}");
            }
        }
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        {
            SparrohPlugin.Logger?.LogWarning($"[BoardingTrident] PersistEquippedCatalogId: {ex.Message}");
        }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[BoardingTrident] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[BoardingTrident] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[BoardingTrident] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            if (baseIndex < 0)
            {
                ...
                {
                    if (Global.Instance.AllGear[g] is WideGun &&
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[BoardingTrident] OnClose: no base index for custom equip slot {i}.");
                continue;
            }
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[BoardingTrident] OnClose stamp: {ex.Message}");
            }
        }
