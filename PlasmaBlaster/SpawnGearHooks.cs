...
///  2. Postfix stamps our GearInfo + PlasmaBlasterBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[PlasmaBlaster] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        //SparrohPlugin.Logger?.LogInfo(
            //$"[PlasmaBlaster] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[PlasmaBlaster] Cannot remap spawn — base gun AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        SparrohPlugin.Logger?.LogInfo(
            $"[PlasmaBlaster] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        // Prefer ScoutLaserRifle specifically.
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[PlasmaBlaster] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[PlasmaBlaster] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[PlasmaBlaster] Catalog gear missing during stamp.");
            return;
        }
    ...
    /// and re-run IGear.ApplyUpgrades so GunData restores from our catalog baseline.
    /// </summary>
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            PlasmaBlasterBehaviour behaviour = live.gameObject.GetComponent<PlasmaBlasterBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<PlasmaBlasterBehaviour>();

            PlasmaBlasterBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<PlasmaBlasterBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[PlasmaBlaster] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }
        ...
        if (live is Gun liveGun && catalog is Gun catalogGun)
        {
            ...
            if (NeedsStatRepair(liveGun, catalogGun))
                WeaponRegistration.ApplyPlasmaBlasterStats(liveGun, SparrohPlugin.Logger);

            // Design locks after ApplyUpgrades.
            liveGun.IsAimEnabled = PlasmaBlasterBalance.IsAimEnabled;
            liveGun.GunData.automatic = PlasmaBlasterBalance.Automatic;
            liveGun.GunData.useAmmoOnFire = PlasmaBlasterBalance.UseAmmoOnFire;
            liveGun.GunData.hitForce = PlasmaBlasterBalance.HitForce;
            liveGun.GunData.damageEffect = PlasmaBlasterBalance.DamageEffect;
            if (liveGun.GunData.damageEffectAmount <= 0f)
                liveGun.GunData.damageEffectAmount = PlasmaBlasterBalance.DamageEffectAmount;

            if (liveGun is ScoutLaserRifle scout && scout.IsLaserModeActive)
            {
                try { scout.IsLaserModeActive = false; }
                catch { /* ignore */ }
            }

            if (PlasmaBlasterBehaviour.TryGet(liveGun, out var pbLive))
                pbLive.OnUpgradesApplied(liveGun);

            WeaponRegistration.EnsureProjectileBullet(liveGun, SparrohPlugin.Logger);
        }

        PersistEquippedCatalogId(catalog, slot);

        SparrohPlugin.Logger?.LogInfo(
            $"[PlasmaBlaster] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
            $"(Info={live.Info?.APIName}, Prefab={(live.Prefab as Component)?.name}).");
    }

    private static bool NeedsStatRepair(Gun live, Gun catalog)
    {
        if (live == null || catalog == null)
            return false;

        // Catalog has Decay + our damage; Scout baseline is Normal / different mag.
        if (catalog.GunData.damageEffect == EffectType.Decay &&
            live.GunData.damageEffect != EffectType.Decay)
            return true;

        if (Mathf.Abs(live.GunData.damage - catalog.GunData.damage) > 0.5f &&
            catalog.GunData.damage >= 20f && live.GunData.damage < 15f)
            return true;

        if (live.GunData.magazineSize != catalog.GunData.magazineSize &&
            catalog.GunData.magazineSize == PlasmaBlasterBalance.MagazineSize)
            return true;
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[PlasmaBlaster] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[PlasmaBlaster] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[PlasmaBlaster] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[PlasmaBlaster] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[PlasmaBlaster] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            if (baseIndex < 0)
            {
                ...
                {
                    if (Global.Instance.AllGear[g] is ScoutLaserRifle &&
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[PlasmaBlaster] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[PlasmaBlaster] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[PlasmaBlaster] OnClose stamp: {ex.Message}");
            }
        }
