...
///
/// Strategy:
///  1. Prefix with <c>ref int allGearIndex</c> so Harmony writes the remapped Globbler index.
///  2. Postfix: stamp GearInfo + ManifoldRocketBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[ManifoldRocket] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[ManifoldRocket] Cannot remap spawn — base Globbler AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        SparrohPlugin.Logger?.LogInfo(
            $"[ManifoldRocket] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        // Prefer Globbler specifically.
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
            if (Global.Instance.AllGear[i] is Globbler &&
                !IsOurCatalogGear(Global.Instance.AllGear[i]))
            {
                WeaponRegistration.SetBaseAllGearIndex(i);
                return i;
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[ManifoldRocket] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[ManifoldRocket] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[ManifoldRocket] Catalog gear missing during stamp.");
            return;
        }
    ...
    /// <summary>
    /// After NGO spawns vanilla Globbler, rebind the live instance to our catalog identity
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            ManifoldRocketBehaviour behaviour = live.gameObject.GetComponent<ManifoldRocketBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<ManifoldRocketBehaviour>();

            ManifoldRocketBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<ManifoldRocketBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[ManifoldRocket] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        if (live is Gun liveGun)
        {
            // Always re-apply full baseline after stamp — ApplyUpgrades can restore Globbler stats.
            WeaponRegistration.ApplyManifoldRocketStats(liveGun, SparrohPlugin.Logger);

            liveGun.IsAimEnabled = MrBalance.IsAimEnabled;
            liveGun.GunData.automatic = MrBalance.Automatic;
            liveGun.GunData.hitForce = MrBalance.HitForce;

            if (ManifoldRocketBehaviour.TryGet(liveGun, out var mr))
                mr.OnUpgradesApplied(liveGun);

            SparrohPlugin.Logger?.LogInfo(
                $"[ManifoldRocket] Live stats dmg={liveGun.GunData.damage} " +
                $"force={liveGun.GunData.hitForce} mag={liveGun.GunData.magazineSize} " +
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[ManifoldRocket] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
            $"(Info={live.Info?.APIName}, Prefab={(live.Prefab as Component)?.name}).");
    }

    private static bool NeedsManifoldStatRepair(Gun live, Gun catalog)
    {
        if (live == null || catalog == null)
            return false;

        if (catalog.GunData.magazineSize == MrBalance.MagazineSize &&
            live.GunData.magazineSize != MrBalance.MagazineSize)
            return true;
        ...
        if (live.IsAimEnabled != MrBalance.IsAimEnabled)
            return true;

        if (live.GunData.hitForce > 0.01f)
            return true;

        if (Mathf.Abs(live.GunData.fireInterval - MrBalance.FireInterval) > 0.05f)
            return true;
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[ManifoldRocket] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[ManifoldRocket] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[ManifoldRocket] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[ManifoldRocket] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[ManifoldRocket] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[ManifoldRocket] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[ManifoldRocket] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[ManifoldRocket] OnClose stamp: {ex.Message}");
            }
        }
