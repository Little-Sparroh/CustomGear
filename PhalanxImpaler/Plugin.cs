...
/// <summary>
/// Phalanx Impaler — melee kit for Mycopunk (Phase 0–3: registration + empty-grid combat).
...
{
    public const string PluginGUID = "sparroh.phalanximpaler";
    public const string PluginName = "PhalanxImpaler";
    public const string PluginVersion = "0.1.0";

    /// <summary>Stable GearInfo.ID — design doc 93000.</summary>
    public const int GearId = 94700;


    public const string GearApiName = "phalanx_impaler";
    public const string GearDisplayName = "Phalanx Impaler";
    public const string GearDescription =
        "Issue pike and buckler. No ammo, no reload — thrust string, raise the plate, throw the shaft. " +
        "Hold the line at long melee reach; pin stragglers with the javelin.";
    ...
    private void Awake()
    {
        ...
        try
        {
            ...
            _harmony.PatchAll(typeof(PhalanxImpalerCombatHooks));
            _harmony.PatchAll(typeof(PhalanxImpalerGuardHooks));
            _harmony.PatchAll(typeof(PhalanxImpalerThrowHooks));
    ...
    internal void TryRegisterGear(string reason)
    {
        ...
        try
        {
            ...
            //Logger.LogInfo(
                //$"[PhalanxImpaler] Registered '{GearDisplayName}' " +
                //$"(api={GearApiName}, id={GearId}) via {reason}. " +
                //$"MeleeRework={(MeleeReworkBridge.IsAvailable ? "yes" : "no")}.");
        }
        catch (Exception ex)
        {
            Logger.LogError($"[PhalanxImpaler] Gear registration failed: {ex}");
        }
    }
...
internal static class PlayerDataPersistenceHooks
{
    ...
    private static void Postfix()
    {
        try
        {
            ...
            if (gear == null)
            {
                SparrohPlugin.Logger?.LogDebug(
                    "[PhalanxImpaler] Persistence: gear not ready at OnAwake (will bind from player melee).");
                return;
            }
            ...
            if (gd != null)
            {
                ...
                //SparrohPlugin.Logger?.LogInfo(
                    //$"[PhalanxImpaler] Persistence OK: level={gd.Level} unlocked={gd.IsUnlocked} " +
        ...
        {
            SparrohPlugin.Logger?.LogError($"[PhalanxImpaler] Persistence postfix failed: {ex}");
        }
    }
}
