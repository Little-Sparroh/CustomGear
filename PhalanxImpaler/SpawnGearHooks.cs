...
/// then stamp Phalanx Impaler identity + behaviour + balance stats.
/// </summary>
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[PhalanxImpaler] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        //SparrohPlugin.Logger?.LogInfo(
            //$"[PhalanxImpaler] Patched {target.Name} ({ps.Length} params) with {prefix.Name}.");
    }
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[PhalanxImpaler] Cannot remap spawn — base MeleeGear AllGear index unknown.");
            return;
        }

        SparrohPlugin.Logger?.LogInfo(
            $"[PhalanxImpaler] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        // Prefer any non-ours MeleeGear in AllGear (Fists after MeleeRework inject).
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[PhalanxImpaler] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[PhalanxImpaler] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[PhalanxImpaler] Catalog gear missing during stamp.");
            return;
        }
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            PhalanxImpalerBehaviour behaviour = live.gameObject.GetComponent<PhalanxImpalerBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<PhalanxImpalerBehaviour>();

            PhalanxImpalerBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<PhalanxImpalerBehaviour>();
        ...
        if (live is MeleeGear melee)
            WeaponRegistration.ApplyImpalerStats(melee);

        try
        {
            live.ApplyUpgrades();
            if (live is MeleeGear melee2)
                WeaponRegistration.ApplyImpalerStats(melee2);
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogError(
                $"[PhalanxImpaler] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[PhalanxImpaler] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
