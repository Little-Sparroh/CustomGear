...
/// </summary>
[HarmonyPatch]
internal static class MarksmanLaserRiflePatches
{
    ...
    private static readonly FieldInfo LastPressedFireTimeField =
        AccessTools.Field(typeof(Gun), "lastPressedFireTime");

    private static readonly FieldInfo LoopFireSoundField =
        AccessTools.Field(typeof(Gun), "loopFireSound");

    private static readonly FieldInfo FireSoundField =
        AccessTools.Field(typeof(Gun), "fireSound");

    private static readonly FieldInfo DefaultSoundField =
        AccessTools.Field(typeof(ScoutLaserRifle), "defaultSound");

    private static readonly FieldInfo LaserFireSoundField =
        AccessTools.Field(typeof(ScoutLaserRifle), "laserFireSound");

    private static readonly FieldInfo LaserBulletPrefabField =
        AccessTools.Field(typeof(ScoutLaserRifle), "laserBulletPrefab");

    private static readonly FieldInfo LaserBulletPrefabGoField =
        AccessTools.Field(typeof(ScoutLaserRifle), "_laserBulletPrefab");

    private static readonly FieldInfo LaserBulletPoolField =
        AccessTools.Field(typeof(ScoutLaserRifle), "laserBulletPool");

    private static readonly FieldInfo DefaultLaserBulletPrefabField =
        AccessTools.Field(typeof(ScoutLaserRifle), "defaultLaserBulletPrefab");

    private static readonly FieldInfo DefaultLaserBulletPoolField =
        AccessTools.Field(typeof(ScoutLaserRifle), "defaultLaserBulletPool");
    ...
    private static bool _loggedMissingContinuousLaser;

    // TEMP laser diagnostics — remove once RMB beam works.
    private const bool LaserDebugLogs = true;
    private const float LaserDebugInterval = 0.5f;
    private static float _nextLaserDebugTime;
    private static bool _loggedGateOk;
    private static bool _loggedGateNoBehaviour;
    private static bool _loggedGateNotOurs;
    private static int _lastDebugKey = int.MinValue;
    private static bool _loggedBulletType;
    ...
    private static void GunUpdate_Prefix(Gun __instance)
    {
        ...
        {
            if (LaserDebugLogs && !_loggedGateNotOurs)
            {
                // Only note once if this looks like a stamped Marksman (has behaviour) but IsOurGear failed.
                if (scout.GetComponent<MarksmanLaserRifleBehaviour>() != null)
                {
                    _loggedGateNotOurs = true;
                    LaserLog(
                        $"GATE fail IsOurGear=false but behaviour present " +
                        $"api={scout.Info?.APIName} id={scout.Info?.ID} " +
                        $"prefabApi={(scout.Prefab as IUpgradable)?.Info?.APIName}");
                }
            }
            return;
        }
        ...
        if (!_loggedGateOk)
        {
            _loggedGateOk = true;
            LaserLog(
                $"GATE ok owner={scout.IsOwner} active={scout.Active} " +
                $"api={scout.Info?.APIName} id={scout.Info?.ID}");
        }
    ...
    {
        if (!MarksmanLaserRifleBehaviour.TryGet(scout, out MarksmanLaserRifleBehaviour behaviour))
        {
            if (LaserDebugLogs && !_loggedGateNoBehaviour)
            {
                _loggedGateNoBehaviour = true;
                LaserLog("GATE fail TryGet behaviour=null on our gear");
            }
            return;
        }
        ...
        try
        {
            capacity = scout.LaserData.laserChargeCapacity;
            minCharge = scout.LaserData.MinLaserChargeNonNormalized;
        }
        catch { /* ignore */ }
        ...
        if (!hotswap)
        {
            ...
            else
            {
                ...
                wantFire = fireHeld;
                if (aimHeld && charge <= 0f)
                    blockReason = "aimHeld but charge<=0";
            }
        }
        else
        {
            ...
            else
            {
                wantLaser = false;
                wantFire = false;
                if (fireHeld && charge <= 0f)
                    blockReason = "hotswap fireHeld but charge<=0";
            }
        }
        ...
        if (wantLaser && !scout.IsLaserModeActive)
        {
            if (minCharge > 0f && charge < minCharge)
            {
                ...
                blockReason = $"minCharge gate charge={charge:0.##} min={minCharge:0.##}";
            }
        }
        ...
        if (wantLaser && charge <= 0f)
        {
            wantLaser = false;
            wantFire = hotswap ? false : fireHeld;
            blockReason = "empty charge while wanting laser";
        }
        ...
        if (hotswap && wantLaser && !fireHeld)
        {
            wantLaser = false;
            wantFire = false;
            blockReason = "hotswap wantLaser without fireHeld";
        }

        behaviour.wantLaserMode = wantLaser;

        bool wasLaser = scout.IsLaserModeActive;
        if (wasLaser != wantLaser)
        {
            LaserLog(
                $"MODE change {wasLaser} → {wantLaser} aim={aimHeld} fire={fireHeld} " +
                $"charge={charge:0.##}/{capacity:0.##} hotswap={hotswap} reason={blockReason ?? "ok"}");
            SetLaserMode(scout, wantLaser);
        }

        // Secondary button must drive fire without LMB.
        // Vanilla laser is mode-toggle + LMB; we never press LMB for RMB laser, so we must
        // fully synthesize the fire-held path (forceEnableFire alone is not enough for
        // continuous beam start / sprint gates / lastPressedFireTime).
        ...
        ApplyForcedFireState(scout, needForceFire, fireHeld);
        ...
        if (hotswap && !wantLaser)
        {
            ...
            if (!needForceFire && !fireHeld)
                ApplyForcedFireState(scout, force: false, realFireHeld: false);
        }

        // One-shot diagnostic if laser mode is on but bullet prefab is not continuous.
        if (wantLaser && scout.IsLaserModeActive && !_loggedMissingContinuousLaser)
        {
            try
            {
                if (!scout.IsContinuous())
                {
                    _loggedMissingContinuousLaser = true;
                    LaserLog(
                        "WARN IsContinuous()=false after laser mode — bullet prefab/pool wrong");
                }
            }
            catch (Exception ex)
            {
                LaserLog($"WARN IsContinuous() threw: {ex.Message}");
            }
        }

        // Throttled status while player is trying to laser (or mode is active).
        if (LaserDebugLogs && (aimHeld || wantLaser || scout.IsLaserModeActive || needForceFire))
        {
            int key = (aimHeld ? 1 : 0) | (fireHeld ? 2 : 0) | (wantLaser ? 4 : 0) |
                      (wantFire ? 8 : 0) | (needForceFire ? 16 : 0) |
                      (scout.IsLaserModeActive ? 32 : 0) | (scout.IsFiring ? 64 : 0) |
                      (scout.WantsToFire ? 128 : 0);
            float now = Time.unscaledTime;
            if (key != _lastDebugKey || now >= _nextLaserDebugTime)
            {
                _lastDebugKey = key;
                _nextLaserDebugTime = now + LaserDebugInterval;
                bool cont = false;
                try { cont = scout.IsContinuous(); } catch { /* ignore */ }
                LaserLog(
                    $"TICK aim={aimHeld} fire={fireHeld} wantL={wantLaser} wantF={wantFire} " +
                    $"force={needForceFire} mode={scout.IsLaserModeActive} " +
                    $"wantsFire={scout.WantsToFire} canFire={scout.CanFire} isFiring={scout.IsFiring} " +
                    $"cont={cont} charge={charge:0.##}/{capacity:0.##} min={minCharge:0.##} " +
                    $"ammo={ammo:0.##} auto={scout.GunData.automatic} useAmmo={scout.GunData.useAmmoOnFire} " +
                    $"fi={scout.GunData.fireInterval:0.###} hotswap={hotswap} " +
                    $"block={blockReason ?? "-"} " +
                    $"synthFireHeld={GetBool(IsFireInputHeldField, scout)} " +
                    $"forceField={GetBool(ForceEnableFireField, scout)}");
            }
        }
    }

    private static void LaserLog(string msg)
    {
        if (!LaserDebugLogs)
            return;
        SparrohPlugin.Logger?.LogInfo($"[DMLRRework][Laser] {msg}");
    }


    /// <summary>
    /// Drive Gun.Update's WantsToFire path when fire comes from RMB (or other non-LMB).
    /// Sets forceEnableFire + isFireInputHeld + lastPressedFireTime so continuous laser can start.
    /// </summary>
    private static void ApplyForcedFireState(ScoutLaserRifle scout, bool force, bool realFireHeld)
    {
        if (force)
        {
            SetBool(ForceEnableFireField, scout, true);
            SetBool(IsFireInputHeldField, scout, true);
            try
            {
                LastPressedFireTimeField?.SetValue(scout, Time.time);
            }
            catch
            {
                // ignore
        ...
        else
        {
            ...
            // Only clear synthetic hold when LMB is not actually down.
            if (!realFireHeld)
                SetBool(IsFireInputHeldField, scout, false);
        }
    }
    ...
    private static void SetLaserMode(ScoutLaserRifle scout, bool laser)
    {
        ...
        // ApplyUpgrades / leaving laser calls ResetBullet(), which Disposes laserBulletPool.
        // Entering laser then SetBullet(prefab, disposedPool) → no LaserBullet cylinder.
        // Recreate a fresh pool (and ensure prefab) before the mode setter swaps bullets.
        if (laser)
            EnsureLaserBulletPool(scout);

        // IsLaserModeActive setter: bullet pool, gunData auto/ammo/interval, RPCs.
        scout.IsLaserModeActive = laser;

        // Owner path never sets loopFireSound/fireSound (only net sync does for non-owners).
        // Continuous laser still works without loop, but match observer setup for consistency.
        ApplyLaserAudio(scout, laser);
        ...
        if (!laser)
        {
            ...
            _loggedBulletType = false;
            LaserLog("MODE exit complete");
        }
        else
        {
            // Ensure laser mode gunData sticks even if something rewrote automatic after setter.
            scout.GunData.automatic = 0;
            scout.GunData.useAmmoOnFire = 0;
            scout.GunData.burstSize = 0;
            scout.GunData.fireInterval = scout.LaserData.laserFireInterval;

            bool cont = false;
            try { cont = scout.IsContinuous(); } catch (Exception ex) { LaserLog($"IsContinuous threw: {ex.Message}"); }
            LaserLog(
                $"MODE enter done mode={scout.IsLaserModeActive} cont={cont} " +
                $"auto={scout.GunData.automatic} useAmmo={scout.GunData.useAmmoOnFire} " +
                $"fi={scout.GunData.fireInterval:0.###} charge={scout.LaserCharge:0.##}");
        }
    ...
    /// <summary>
    /// Rebuild Scout laser bullet prefab + ObjectPool after vanilla ResetBullet disposed the pool.
    /// Must run before IsLaserModeActive=true (setter calls SetBullet with these fields).
    /// </summary>
    private static void EnsureLaserBulletPool(ScoutLaserRifle scout)
    {
        try
        {
            IBullet prefab = LaserBulletPrefabField?.GetValue(scout) as IBullet;
            string prefabSrc = prefab != null ? "laserBulletPrefab" : null;
            if (prefab == null)
            {
                prefab = DefaultLaserBulletPrefabField?.GetValue(scout) as IBullet;
                if (prefab != null)
                    prefabSrc = "defaultLaserBulletPrefab";
            }

            if (prefab == null && LaserBulletPrefabGoField?.GetValue(scout) is GameObject go)
            {
                prefab = go.GetComponent<IBullet>();
                if (prefab != null)
                {
                    prefabSrc = $"_laserBulletPrefab GO={go.name}";
                    LaserBulletPrefabField?.SetValue(scout, prefab);
                    if (DefaultLaserBulletPrefabField != null &&
                        DefaultLaserBulletPrefabField.GetValue(scout) == null)
                    {
                        DefaultLaserBulletPrefabField.SetValue(scout, prefab);
                    }
                }
                else
                {
                    LaserLog($"POOL _laserBulletPrefab GO={go.name} has no IBullet");
                }
            }
            else if (prefab == null)
            {
                object goObj = LaserBulletPrefabGoField?.GetValue(scout);
                LaserLog($"POOL _laserBulletPrefab field={(goObj == null ? "null" : goObj.GetType().Name)}");
            }

            if (prefab == null)
            {
                LaserLog("POOL FAIL laser bullet prefab missing — beam cannot spawn");
                return;
            }

            // Always recreate: disposed pools throw / yield nothing; CreateBullet uses
            // the gun's current bulletPrefab at Get() time (set by IsLaserModeActive).
            var pool = scout.CreateBulletPool();
            LaserBulletPoolField?.SetValue(scout, pool);

            // Keep default* in sync if never initialized or previously disposed.
            if (DefaultLaserBulletPoolField != null)
            {
                object existingDefault = DefaultLaserBulletPoolField.GetValue(scout);
                if (existingDefault == null)
                    DefaultLaserBulletPoolField.SetValue(scout, pool);
            }
            ...
            if (LaserBulletPrefabField != null && LaserBulletPrefabField.GetValue(scout) == null)
                LaserBulletPrefabField.SetValue(scout, prefab);

            string typeName = prefab.GetType().Name;
            bool isCont = false;
            try { isCont = prefab.IsContinuous; } catch { /* ignore */ }
            LaserLog($"POOL ok prefab={typeName} src={prefabSrc} continuous={isCont} poolRecreated=true");
        }
        catch (Exception ex)
        {
            LaserLog($"POOL FAIL exception: {ex}");
        }
    }



    private static void ApplyLaserAudio(ScoutLaserRifle scout, bool laser)
    {
        try
        {
            if (LoopFireSoundField != null)
                LoopFireSoundField.SetValue(scout, laser);

            if (FireSoundField == null)
                return;

            if (laser)
            {
                object laserSnd = LaserFireSoundField?.GetValue(scout);
                if (laserSnd != null)
                    FireSoundField.SetValue(scout, laserSnd);
            }
            else
            {
                object def = DefaultSoundField?.GetValue(scout);
                if (def != null)
                    FireSoundField.SetValue(scout, def);
            }
        }
        catch
        {
            // ignore audio reflection failures
        }
    }
    ...
    // -------------------------------------------------------------------------
    // Hide vanilla "Hold R" full-charge popup (Marksman uses RMB for laser)
    // -------------------------------------------------------------------------
    ...
    {
        if (HudGearField?.GetValue(__instance) is not IUpgradable gear)
            return;
        if (!SparrohPlugin.IsOurGear(gear))
            return;

        if (NoChargePopupField?.GetValue(__instance) is GameObject popup && popup.activeSelf)
    ...
    private static void AfterUpgradesEnabled_Postfix(ScoutLaserRifle __instance)
    {
        ...
        int auto = 1;
        if (MarksmanLaserRifleBehaviour.TryGet(__instance, out MarksmanLaserRifleBehaviour b) &&
        ...
        BulletFlags flags)
    {
        if (!SparrohPlugin.IsOurGear(__instance))
            return;
        if (!MarksmanLaserRifleBehaviour.TryGet(__instance, out MarksmanLaserRifleBehaviour behaviour))
            return;
    ...
    private static void ApplyCondensedMunitions(
        ...
        ref BulletData data,
        ref MarksmanLaserRifleBehaviour.Data wd)
    {
        if (!MarksmanLaserRifleBehaviour.TryGet(scout, out MarksmanLaserRifleBehaviour behaviour))
            return;
        ...
        ref BulletData bulletData)
    {
        ...
        // TEMP: log first bullet type after entering laser (or any fire while debugging).
        if (LaserDebugLogs && __instance.IsLaserModeActive && !_loggedBulletType)
        {
            _loggedBulletType = true;
            LaserLog(
                $"BULLET fired type={bullet?.GetType().Name ?? "null"} " +
                $"isLaserBullet={bullet is LaserBullet} mode={__instance.IsLaserModeActive} " +
                $"cont={__instance.IsContinuous()}");
        }
    ...
    private static void NotifyModeFired(MarksmanLaserRifleBehaviour b, bool modeLaser)
    {
    ...
    private static void GunUpdate_Postfix_LaserEffects(Gun __instance)
    {
        ...
        if (!SparrohPlugin.IsOurGear(scout))
            return;
        if (!MarksmanLaserRifleBehaviour.TryGet(scout, out MarksmanLaserRifleBehaviour b))
            return;

        ref MarksmanLaserRifleBehaviour.Data wd = ref b.WeaponData;
    ...
    /// </summary>
    private static void TickGravitationalPull(ScoutLaserRifle scout, ref MarksmanLaserRifleBehaviour.Data wd)
    {
        try
        {
            if (!MarksmanLaserRifleBehaviour.TryGet(scout, out MarksmanLaserRifleBehaviour behaviour))
    ...
    private static void FireIncendiaryWave(ScoutLaserRifle scout, ref MarksmanLaserRifleBehaviour.Data wd)
    {
        try
        {
    ...
    /// </summary>
    private static void StampWeaponElements(ScoutLaserRifle scout, ref MarksmanLaserRifleBehaviour.Data wd)
    {
        try
        {
            if (wd.tripleElement && wd.tripleEffect > EffectType.Normal)
    ...
    private static void ThrowVoltaicBattery(ScoutLaserRifle scout, MarksmanLaserRifleBehaviour b, float power)
    {
        ref MarksmanLaserRifleBehaviour.Data wd = ref b.WeaponData;
        float now = Time.time;
    ...
    private static void GunDisable_Postfix(Gun __instance)
    {
        ...
        SetBool(IsFireInputHeldField, __instance, false);
        if (MarksmanLaserRifleBehaviour.TryGet(__instance, out MarksmanLaserRifleBehaviour b))
