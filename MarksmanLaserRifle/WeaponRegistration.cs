...
///  5. Attaches <see cref="DmlrReworkBehaviour"/> for custom data / upgrade hooks
...
public static class WeaponRegistration
{
        ...
        out IUpgradable registeredGear)
    {
        ...
        DmlrReworkBehaviour behaviour = clone.GetComponent<DmlrReworkBehaviour>();
        if (behaviour == null)
            behaviour = clone.AddComponent<DmlrReworkBehaviour>();
    ...
    /// <summary>
    /// Rewrites GunData / LaserGunData / aim fields from <see cref="DmlrBalance"/>.
    ...
    public static void ApplyMarksmanStats(Gun gun, ManualLogSource log = null)
    {
        ...
        // --- Core combat (DMR) ---
        g.damage = DmlrBalance.Damage;
        g.damageEffect = DmlrBalance.DamageEffect;
        g.damageEffectAmount = DmlrBalance.DamageEffectAmount;
        g.fireInterval = DmlrBalance.FireInterval;
        g.fireAnimationSpeedMultiplier = DmlrBalance.FireAnimationSpeedMultiplier;
        g.automatic = DmlrBalance.Automatic;
        g.bulletsPerShot = DmlrBalance.BulletsPerShot;
        g.burstSize = DmlrBalance.BurstSize;
        g.burstFireInterval = DmlrBalance.BurstFireInterval;
        g.useAmmoOnFire = DmlrBalance.UseAmmoOnFire;
        g.doesEachBulletInShotRemoveAmmo = DmlrBalance.DoesEachBulletInShotRemoveAmmo;
        g.doesEachBulletInShotTriggerEffects = DmlrBalance.DoesEachBulletInShotTriggerEffects;
        g.hitForce = DmlrBalance.HitForce;
        g.hitVFXSize = DmlrBalance.HitVfxSize;

        // --- Magazine / reserves ---
        g.magazineSize = DmlrBalance.MagazineSize;
        g.hasLimitedAmmo = DmlrBalance.HasLimitedAmmo;
        g.ammoCapacity = DmlrBalance.AmmoCapacity;
        g.ammoCollectMultiplier = DmlrBalance.AmmoCollectMultiplier;
        g.storedAmmoCollectMultiplier = DmlrBalance.StoredAmmoCollectMultiplier;
        g.ammoGenerationEfficiency = DmlrBalance.AmmoGenerationEfficiency;
        g.useAmmoWhileFiringInterval = DmlrBalance.UseAmmoWhileFiringInterval;
        g.refillAmmoOnReload = DmlrBalance.RefillAmmoOnReload;
        g.reloadDuration = DmlrBalance.ReloadDuration;
        g.autoReloadWhenEmpty = DmlrBalance.AutoReloadWhenEmpty;

        // --- Projectile ---
        g.bulletSpeed = DmlrBalance.BulletSpeed;
        g.bulletGravity = DmlrBalance.BulletGravity;
        g.maxBounces = DmlrBalance.MaxBounces;
        g.bulletMagnetismSurface = DmlrBalance.BulletMagnetismSurface;
        g.bulletMagnetismTarget = DmlrBalance.BulletMagnetismTarget;
        g.bulletShakeTranslation = DmlrBalance.BulletShakeTranslation;
        g.bulletShakeRotation = DmlrBalance.BulletShakeRotation;

        // --- Range / falloff ---
        g.rangeData.falloffStartDistance = DmlrBalance.FalloffStartDistance;
        g.rangeData.falloffEndDistance = DmlrBalance.FalloffEndDistance;
        g.rangeData.maxDamageRange = DmlrBalance.MaxDamageRange;
        g.rangeData.maxFalloffDamageMultiplier = DmlrBalance.MaxFalloffDamageMultiplier;

        // --- Spread ---
        g.spreadData.spreadType = DmlrBalance.SpreadType;
        g.spreadData.spreadSize = DmlrBalance.SpreadSize;
        g.firstShotSpreadMultiplier = DmlrBalance.FirstShotSpreadMultiplier;

        // --- Recoil ---
        g.recoilData.recoilX = DmlrBalance.RecoilX;
        g.recoilData.recoilY = DmlrBalance.RecoilY;
        g.recoilData.recoilZ = DmlrBalance.RecoilZ;
        g.recoilData.maxRecoilZ = DmlrBalance.MaxRecoilZ;
        g.recoilData.translateZ = DmlrBalance.TranslateZ;
        g.recoilData.maxTranslateZ = DmlrBalance.MaxTranslateZ;
        g.recoilData.aimTranslateMultiplier = DmlrBalance.AimTranslateMultiplier;
        g.recoilData.recoilSpeed = DmlrBalance.RecoilSpeed;
        g.recoilData.recoilRecoverySpeed = DmlrBalance.RecoilRecoverySpeed;
        g.recoilData.translateSpeed = DmlrBalance.TranslateSpeed;
        g.recoilData.translateRecoverySpeed = DmlrBalance.TranslateRecoverySpeed;
        g.recoilData.recoilTargetDecaySpeed = DmlrBalance.RecoilTargetDecaySpeed;
        g.recoilData.aimRecoilMultiplier = DmlrBalance.AimRecoilMultiplier;

        // --- Charge (disabled — laser uses LaserGunData) ---
        g.chargeData.duration = DmlrBalance.ChargeDuration;
        g.chargeData.coolDownSpeed = DmlrBalance.ChargeCoolDownSpeed;
        g.chargeData.fireWhenFullyCharged = DmlrBalance.ChargeFireWhenFullyCharged;
        g.chargeData.fireOnRelease = DmlrBalance.ChargeFireOnRelease;
        g.chargeData.canFireWhileCharging = DmlrBalance.ChargeCanFireWhileCharging;
        g.chargeData.time = 0f;

        // --- Fire constraints ---
        g.fireConstraints.canFireWhileSprinting = DmlrBalance.CanFireWhileSprinting;
        g.fireConstraints.canFireWhileSliding = DmlrBalance.CanFireWhileSliding;
        g.fireConstraints.canAimWhileSliding = DmlrBalance.CanAimWhileSliding;
        g.fireConstraints.canAimWhileReloading = DmlrBalance.CanAimWhileReloading;
        g.fireConstraints.canReloadWhileSprinting = DmlrBalance.CanReloadWhileSprinting;

        // --- ADS (design: off — RMB is laser hold) ---
        gun.IsAimEnabled = DmlrBalance.IsAimEnabled;
        gun.AimFOV = DmlrBalance.AimFov;
        TrySetAimTransitionDuration(gun, DmlrBalance.AimTransitionDuration);
        ...
        if (gun is ScoutLaserRifle scout)
        {
            ...
            laser.laserAmmoUseInterval = DmlrBalance.LaserAmmoUseInterval;
            laser.laserDamage = DmlrBalance.LaserDamageData;
            laser.maxLaserBounces = DmlrBalance.MaxLaserBounces;
            laser.laserMagnetismSurface = DmlrBalance.LaserMagnetismSurface;
            laser.laserMagnetismTarget = DmlrBalance.LaserMagnetismTarget;

            laser.laserRangeData.falloffStartDistance = DmlrBalance.LaserFalloffStartDistance;
            laser.laserRangeData.falloffEndDistance = DmlrBalance.LaserFalloffEndDistance;
            laser.laserRangeData.maxDamageRange = DmlrBalance.LaserMaxDamageRange;
            laser.laserRangeData.maxFalloffDamageMultiplier = DmlrBalance.LaserMaxFalloffDamageMultiplier;

            laser.laserFireInterval = DmlrBalance.LaserFireInterval;
            laser.laserChargeCapacity = DmlrBalance.LaserChargeCapacity;
            laser.laserChargeOnHit = DmlrBalance.ResolveLaserChargeOnHit(DmlrBalance.LaserChargeCapacity);
            laser.laserChargeUsePerSecond = DmlrBalance.LaserChargeUsePerSecond;
            laser.laserAmmoRefill = DmlrBalance.LaserAmmoRefill;
            laser.maxMagazineSizeMultiplierFromAmmoRefill = DmlrBalance.MaxMagazineSizeMultiplierFromAmmoRefill;
            laser.minLaserCharge = DmlrBalance.MinLaserCharge;
        ...
        else
        {
            ...
            log?.LogInfo(
                ...
                $"mag={g.magazineSize}, automatic={g.automatic}, aim={DmlrBalance.IsAimEnabled}.");
        }
    }
