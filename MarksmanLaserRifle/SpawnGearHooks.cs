...
internal static class SpawnGearHooks
{
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            DmlrReworkBehaviour behaviour = live.gameObject.GetComponent<DmlrReworkBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<DmlrReworkBehaviour>();

            DmlrReworkBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<DmlrReworkBehaviour>();
        ...
        if (live is Gun liveGun)
        {
            ...
            liveGun.IsAimEnabled = DmlrBalance.IsAimEnabled;
            if (!laserActive)
                liveGun.GunData.automatic = DmlrBalance.Automatic;
            else
            {
    ...
    private static bool NeedsMarksmanStatRepair(Gun live, Gun catalog)
    {
        ...
        if (catalog.GunData.magazineSize == DmlrBalance.MagazineSize &&
            live.GunData.magazineSize != DmlrBalance.MagazineSize)
            return true;
        ...
        {
            if (Mathf.Abs(liveScout.LaserData.minLaserCharge - DmlrBalance.MinLaserCharge) > 0.001f &&
                Mathf.Abs(catalogScout.LaserData.minLaserCharge - DmlrBalance.MinLaserCharge) < 0.001f)
                return true;
        }
