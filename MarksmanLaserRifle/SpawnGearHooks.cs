...
///  2. Postfix: stamp our GearInfo + DmlrReworkBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[DMLRRework] Could not find Player.SpawnGear_Server to patch.");
        ...
        //SparrohPlugin.Logger?.LogInfo(
            //$"[DMLRRework] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[DMLRRework] Cannot remap spawn — base gun AllGear index unknown. " +
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[DMLRRework] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[DMLRRework] Post-spawn stamp failed: {ex}");
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[DMLRRework] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
        ...
        IUpgradable catalog = SparrohPlugin.CustomWeaponPrefab
            ...
            ?? WeaponRegistration.FindGearSafe(SparrohPlugin.ExampleGearApiName, SparrohPlugin.ExampleGearId);

        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[DMLRRework] Catalog gear missing during stamp.");
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            MarksmanLaserRifleBehaviour behaviour = live.gameObject.GetComponent<MarksmanLaserRifleBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<MarksmanLaserRifleBehaviour>();

            MarksmanLaserRifleBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<MarksmanLaserRifleBehaviour>();

            behaviour.InitializeAsPrefab(
                templateBehaviour != null ? templateBehaviour.Description : SparrohPlugin.ExampleGearDescription);
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[DMLRRework] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        // 4) Design locks only after ApplyUpgrades.
        //    Full DmlrBalance is already on the catalog Prefab; ApplyUpgrades copies it then
        //    stacks equipped upgrades. Re-writing the whole sheet here would wipe upgrade mods.
        //    If catalog stats failed to stick (looks like untouched Scout), re-apply once.
        if (live is Gun liveGun)
        {
            bool laserActive = live is ScoutLaserRifle slr && slr.IsLaserModeActive;

            if (catalog is Gun catalogGun &&
                NeedsMarksmanStatRepair(liveGun, catalogGun))
            {
                WeaponRegistration.ApplyMarksmanStats(liveGun, SparrohPlugin.Logger);
            }

            liveGun.IsAimEnabled = MarksmanLaserRifleBalance.IsAimEnabled;
            if (!laserActive)
                liveGun.GunData.automatic = MarksmanLaserRifleBalance.Automatic;
            else
            {
                // Scout laser mode owns these while beaming.
                liveGun.GunData.automatic = 0;
                liveGun.GunData.useAmmoOnFire = 0;
                if (live is ScoutLaserRifle laserGun)
                {
                    liveGun.GunData.burstSize = 0;
                    liveGun.GunData.fireInterval = laserGun.LaserData.laserFireInterval;
                }
            }
        }
        ...
        // Write the catalog id so the next boot restores our weapon, not vanilla Scout.
        PersistEquippedCatalogId(catalog, slot);

        SparrohPlugin.Logger?.LogInfo(
            $"[DMLRRework] Rebound Gear[{slot}] → {SparrohPlugin.ExampleGearApiName} " +
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[DMLRRework] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[DMLRRework] PersistEquippedCatalogId: {ex.Message}");
        }
    }
    ...
    /// <summary>
    /// True when live GunData still looks like vanilla Scout rather than catalog Marksman baseline.
    /// </summary>
    private static bool NeedsMarksmanStatRepair(Gun live, Gun catalog)
    {
        if (live == null || catalog == null)
            return false;

        // Mag size is a strong identity signal (balance sheet locks 20).
        if (catalog.GunData.magazineSize == MarksmanLaserRifleBalance.MagazineSize &&
            live.GunData.magazineSize != MarksmanLaserRifleBalance.MagazineSize)
            return true;

        // Damage far from catalog baseline (before upgrade mults usually stay near base).
        ...
        if (live is ScoutLaserRifle liveScout && catalog is ScoutLaserRifle catalogScout)
        {
            if (Mathf.Abs(liveScout.LaserData.minLaserCharge - MarksmanLaserRifleBalance.MinLaserCharge) > 0.001f &&
                Mathf.Abs(catalogScout.LaserData.minLaserCharge - MarksmanLaserRifleBalance.MinLaserCharge) < 0.001f)
                return true;
        }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[DMLRRework] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[DMLRRework] Safe OnCloseCallback failed: {ex}");
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[DMLRRework] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[DMLRRework] OnClose: no base index for custom equip slot {i}.");

                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[DMLRRework] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[DMLRRework] OnClose stamp: {ex.Message}");
