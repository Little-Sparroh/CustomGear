...
/// </summary>
public static class MarksmanLaserRifleBalance
{
    // -------------------------------------------------------------------------
    ...
    /// <summary>
    /// Charge-drain tick period while laser is firing.
    /// MUST be > 0 — vanilla OnActiveUpdate loops while (time >= interval) and
    /// subtracts interval each pass; 0 never progresses and OOMs via InvokeOnShotFired.
    /// Drain rate is still LaserChargeUsePerSecond (interval is granularity only).
    /// </summary>
    public const float LaserAmmoUseInterval = 0.08f;


    /// <summary>Provisional beam tick damage (tune vs DMR DPS).</summary>
    public const float LaserDamage = 363f;
    ...
    /// <summary>
    /// Surface ray thickness. Also feeds laser trail setup indirectly via ModifyBulletData.
    /// Must be > 0 for a usable beam silhouette against geometry.
    /// </summary>
    public const float LaserMagnetismSurface = 0.15f;

    /// <summary>
    /// Target ray thickness AND laser trail width (LaserBullet.Simulate → trail.SetWidth).
    /// MUST be > 0 — 0 makes the cylinder invisible and the hit volume a zero-radius ray.
    /// </summary>
    public const float LaserMagnetismTarget = 0.35f;
