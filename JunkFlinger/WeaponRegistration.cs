...
///  5. Attaches <see cref="JunkFlingerBehaviour"/> for custom data / upgrade hooks
...
public static class WeaponRegistration
{
    ...
    {
        // Prefer API name as the stable identity. Never treat "same numeric ID, different API"
        // as a hit — other mods own their own 9xxxx blocks (e.g. FriendinaBox 92100).
        if (CatalogGear?.Info != null)
        {
            if (!string.IsNullOrEmpty(apiName) && CatalogGear.Info.APIName == apiName)
                return CatalogGear;
            if (string.IsNullOrEmpty(apiName) && gearId >= 0 && CatalogGear.Info.ID == gearId)
                return CatalogGear;
        }
        ...
        if (Global.Instance?.AllGear != null)
        {
            ...
            for (int i = 0; i < all.Length; i++)
            {
                ...
                {
                    if (g.Info.APIName == apiName)
                        return g;
                    // Skip ID-only match when apiName was provided — avoids cross-mod collisions.
                    continue;
                }
        ...
        // Last resort: vanilla FindGear by API name only, fully guarded.
        try
        {
        ...
        out IUpgradable registeredGear)
    {
        ...
        // Lookup is API-name-first so we never adopt another mod's gear that shares a numeric id.
        IUpgradable existing = FindGearSafe(apiName, gearId);
        if (existing != null)
        {
            if (existing.Info != null &&
                (!string.Equals(existing.Info.APIName, apiName, StringComparison.Ordinal) ||
                 (gearId >= 0 && existing.Info.ID != gearId)))
            {
                log?.LogError(
                    $"[WeaponRegistration] FindGearSafe returned foreign gear " +
                    $"(api={existing.Info.APIName}, id={existing.Info.ID}) while looking for " +
                    $"api={apiName} id={gearId}. Refusing reuse.");
                return false;
            }
            ...
            {
                ForceGearTypePrimary(existingGun, log);
                // Re-apply balance so hot reload / balance edits stick on the catalog clone.
                ApplyJunkFlingerStats(existingGun, log);
            }
            // Critical for persistence: re-bind GearData.Gear after save load.
            EnsureGearData(existing, autoUnlock, log);
            //log?.LogInfo(
                //$"[WeaponRegistration] Gear '{apiName}' already present — reusing " +
                //$"(id={existing.Info?.ID}, type={(existing is Gun eg ? eg.GearType.ToString() : "?")}).");
            return true;
        }
        ...
        // Gear select filters by Gun.GearType (serialized). Force Primary so we never
        // land in Throwable/Heavy lists if the clone path or base lookup is weird.
        ForceGearTypePrimary(cloneGun, log);
        ...
        // Custom behaviour host — cylinder / junk / scrap pack.
        JunkFlingerBehaviour behaviour = clone.GetComponent<JunkFlingerBehaviour>();
        if (behaviour == null)
            behaviour = clone.AddComponent<JunkFlingerBehaviour>();
        behaviour.InitializeAsPrefab(description);

        // Catalog baseline from JunkFlingerBalance (GunData + aim).
        // Kill→reload is stripped here and again on live instances via JunkFlingerHooks.
        ApplyJunkFlingerStats(cloneGun, log);

        behaviour.EnsureChambers(Mathf.Max(1, cloneGun.GunData.magazineSize), packAll: false);
    ...
    /// <summary>
    /// Rewrites GunData / aim fields from <see cref="JunkFlingerBalance"/>.
    /// Also strips vanilla Lead Flinger kill→reload on FastReloadShotgun clones.
    /// </summary>
    public static void ApplyJunkFlingerStats(Gun gun, ManualLogSource log = null)
    {
        if (gun == null)
            return;
        ...
        // --- Core combat ---
        g.damage = JunkFlingerBalance.Damage;
        g.damageEffect = JunkFlingerBalance.DamageEffect;
        g.damageEffectAmount = JunkFlingerBalance.DamageEffectAmount;
        g.fireInterval = JunkFlingerBalance.FireInterval;
        g.fireAnimationSpeedMultiplier = JunkFlingerBalance.FireAnimationSpeedMultiplier;
        g.automatic = JunkFlingerBalance.Automatic;
        g.bulletsPerShot = JunkFlingerBalance.BulletsPerShot;
        g.burstSize = JunkFlingerBalance.BurstSize;
        g.burstFireInterval = JunkFlingerBalance.BurstFireInterval;
        g.useAmmoOnFire = JunkFlingerBalance.UseAmmoOnFire;
        g.doesEachBulletInShotRemoveAmmo = JunkFlingerBalance.DoesEachBulletInShotRemoveAmmo;
        g.doesEachBulletInShotTriggerEffects = JunkFlingerBalance.DoesEachBulletInShotTriggerEffects;

        // --- Magazine / reserves ---
        g.magazineSize = JunkFlingerBalance.MagazineSize;
        g.hasLimitedAmmo = JunkFlingerBalance.HasLimitedAmmo;
        g.ammoCapacity = JunkFlingerBalance.AmmoCapacity;
        g.ammoCollectMultiplier = JunkFlingerBalance.AmmoCollectMultiplier;
        g.storedAmmoCollectMultiplier = JunkFlingerBalance.StoredAmmoCollectMultiplier;
        g.ammoGenerationEfficiency = JunkFlingerBalance.AmmoGenerationEfficiency;
        g.useAmmoWhileFiringInterval = JunkFlingerBalance.UseAmmoWhileFiringInterval;
        g.refillAmmoOnReload = JunkFlingerBalance.RefillAmmoOnReload;
        g.reloadDuration = JunkFlingerBalance.ReloadDuration;
        g.autoReloadWhenEmpty = JunkFlingerBalance.AutoReloadWhenEmpty;

        // --- Projectile ---
        g.bulletSpeed = JunkFlingerBalance.BulletSpeed;
        g.bulletGravity = JunkFlingerBalance.BulletGravity;
        g.maxBounces = JunkFlingerBalance.MaxBounces;
        g.bulletMagnetismSurface = JunkFlingerBalance.BulletMagnetismSurface;
        g.bulletMagnetismTarget = JunkFlingerBalance.BulletMagnetismTarget;

        // --- Range / falloff ---
        g.rangeData.falloffStartDistance = JunkFlingerBalance.FalloffStartDistance;
        g.rangeData.falloffEndDistance = JunkFlingerBalance.FalloffEndDistance;
        g.rangeData.maxDamageRange = JunkFlingerBalance.MaxDamageRange;
        g.rangeData.maxFalloffDamageMultiplier = JunkFlingerBalance.MaxFalloffDamageMultiplier;

        g.hitForce = JunkFlingerBalance.HitForce;
        g.hitVFXSize = JunkFlingerBalance.HitVfxSize;

        // --- Spread ---
        g.spreadData.spreadType = JunkFlingerBalance.SpreadType;
        g.spreadData.spreadSize = JunkFlingerBalance.SpreadSize;
        g.firstShotSpreadMultiplier = JunkFlingerBalance.FirstShotSpreadMultiplier;

        // --- Recoil ---
        g.recoilData.recoilX = JunkFlingerBalance.RecoilX;
        g.recoilData.recoilY = JunkFlingerBalance.RecoilY;
        g.recoilData.recoilZ = JunkFlingerBalance.RecoilZ;
        g.recoilData.maxRecoilZ = JunkFlingerBalance.MaxRecoilZ;
        g.recoilData.translateZ = JunkFlingerBalance.TranslateZ;
        g.recoilData.maxTranslateZ = JunkFlingerBalance.MaxTranslateZ;
        g.recoilData.aimTranslateMultiplier = JunkFlingerBalance.AimTranslateMultiplier;
        g.recoilData.recoilSpeed = JunkFlingerBalance.RecoilSpeed;
        g.recoilData.recoilRecoverySpeed = JunkFlingerBalance.RecoilRecoverySpeed;
        g.recoilData.translateSpeed = JunkFlingerBalance.TranslateSpeed;
        g.recoilData.translateRecoverySpeed = JunkFlingerBalance.TranslateRecoverySpeed;
        g.recoilData.recoilTargetDecaySpeed = JunkFlingerBalance.RecoilTargetDecaySpeed;
        g.recoilData.aimRecoilMultiplier = JunkFlingerBalance.AimRecoilMultiplier;

        g.bulletShakeTranslation = JunkFlingerBalance.BulletShakeTranslation;
        g.bulletShakeRotation = JunkFlingerBalance.BulletShakeRotation;

        // --- Charge (disabled on base) ---
        g.chargeData.duration = JunkFlingerBalance.ChargeDuration;
        g.chargeData.coolDownSpeed = JunkFlingerBalance.ChargeCoolDownSpeed;
        g.chargeData.fireWhenFullyCharged = JunkFlingerBalance.ChargeFireWhenFullyCharged;
        g.chargeData.fireOnRelease = JunkFlingerBalance.ChargeFireOnRelease;
        g.chargeData.canFireWhileCharging = JunkFlingerBalance.ChargeCanFireWhileCharging;
        g.chargeData.time = 0f;

        // --- Fire constraints ---
        g.fireConstraints.canFireWhileSprinting = JunkFlingerBalance.CanFireWhileSprinting;
        g.fireConstraints.canFireWhileSliding = JunkFlingerBalance.CanFireWhileSliding;
        g.fireConstraints.canAimWhileSliding = JunkFlingerBalance.CanAimWhileSliding;
        g.fireConstraints.canAimWhileReloading = JunkFlingerBalance.CanAimWhileReloading;
        g.fireConstraints.canReloadWhileSprinting = JunkFlingerBalance.CanReloadWhileSprinting;

        // --- ADS ---
        gun.IsAimEnabled = JunkFlingerBalance.IsAimEnabled;
        gun.AimFOV = JunkFlingerBalance.AimFov;
        TrySetAimTransitionDuration(gun, JunkFlingerBalance.AimTransitionDuration);

        // Strip vanilla Lead Flinger kill→reload on the catalog / live FastReloadShotgun.
        if (gun is FastReloadShotgun frs)
        {
            try
            {
            ...
            {
                // publicizer / layout mismatch — live strip still runs in hooks
            }
        }

        //log?.LogInfo(
            //$"[WeaponRegistration] Applied Junk Flinger stats: dmg={g.damage}, " +
            //$"rpm≈{60f / Mathf.Max(0.001f, g.fireInterval):0}, mag={g.magazineSize}, reserve={g.ammoCapacity}, " +
            //$"reload={g.reloadDuration}s, speed={g.bulletSpeed}, falloff={g.rangeData.falloffStartDistance}-{g.rangeData.falloffEndDistance}.");
    }
    ...
    /// <summary>
    /// Gun.GearType is [field: SerializeField] public get; private set.
    /// Menu lists filter with <c>upgradable.GearType == slotType</c>.
    /// </summary>
    private static void ForceGearTypePrimary(Gun gear, ManualLogSource log)
    {
        if (gear == null)
            return;

        if (gear.GearType == GearType.Primary)
            return;

        bool ok = TrySetMember(gear, "<GearType>k__BackingField", GearType.Primary)
                  || TrySetMember(gear, "GearType", GearType.Primary);

        if (ok)
            log?.LogInfo($"[WeaponRegistration] Forced GearType.Primary (was {gear.GearType}).");
        else
            log?.LogWarning(
                $"[WeaponRegistration] Could not set GearType.Primary (still {gear.GearType}). " +
                "Weapon may appear under the wrong gear category.");
    }
...
public static class ModelImportHooks
{
    ...
    public static void ApplyPlaceholderHooks(GameObject gearRoot, ManualLogSource log)
    {
        ...
        log?.LogDebug("[ModelImportHooks] Placeholder only — using vanilla Lead Flinger (FastReloadShotgun) visuals.");
