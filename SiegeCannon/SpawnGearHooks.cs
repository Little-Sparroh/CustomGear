...
///
/// Strategy:
///  1. Prefix with <c>ref int allGearIndex</c> so Harmony writes the remapped MiniCannon index.
///  2. Postfix: stamp GearInfo + SiegeCannonBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[SiegeCannon] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        //SparrohPlugin.Logger?.LogInfo(
            //$"[SiegeCannon] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[SiegeCannon] Cannot remap spawn — base MiniCannon AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        SparrohPlugin.Logger?.LogInfo(
            $"[SiegeCannon] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        // Prefer MiniCannon specifically.
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
            if (Global.Instance.AllGear[i] is MiniCannon &&
                !IsOurCatalogGear(Global.Instance.AllGear[i]))
            {
                WeaponRegistration.SetBaseAllGearIndex(i);
                return i;
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[SiegeCannon] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[SiegeCannon] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[SiegeCannon] Catalog gear missing during stamp.");
            return;
        }
    ...
    /// <summary>
    /// After NGO spawns vanilla MiniCannon, rebind the live instance to our catalog identity
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            SiegeCannonBehaviour behaviour = live.gameObject.GetComponent<SiegeCannonBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<SiegeCannonBehaviour>();

            SiegeCannonBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<SiegeCannonBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[SiegeCannon] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        if (live is Gun liveGun)
        {
            if (catalog is Gun catalogGun && NeedsSiegeStatRepair(liveGun, catalogGun))
                WeaponRegistration.ApplySiegeCannonStats(liveGun, SparrohPlugin.Logger);
            else
                WeaponRegistration.SanitizeMiniCannonBaseline(liveGun, SparrohPlugin.Logger);

            liveGun.IsAimEnabled = ScBalance.IsAimEnabled;
            liveGun.GunData.automatic = ScBalance.Automatic;

            if (SiegeCannonBehaviour.TryGet(liveGun, out var sc))
                sc.OnUpgradesApplied(liveGun);
        }

        PersistEquippedCatalogId(catalog, slot);

        SparrohPlugin.Logger?.LogInfo(
            $"[SiegeCannon] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
            $"(Info={live.Info?.APIName}, Prefab={(live.Prefab as Component)?.name}).");
    }

    private static bool NeedsSiegeStatRepair(Gun live, Gun catalog)
    {
        if (live == null || catalog == null)
            return false;

        if (catalog.GunData.magazineSize == ScBalance.MagazineSize &&
            live.GunData.magazineSize != ScBalance.MagazineSize)
            return true;
        ...
        if (live.IsAimEnabled != ScBalance.IsAimEnabled)
            return true;

        if (live is MiniCannon mini && mini.MiniCannonData.enableSpinUp != ScBalance.EnableSpinUp)
            return true;
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[SiegeCannon] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[SiegeCannon] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[SiegeCannon] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[SiegeCannon] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[SiegeCannon] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            if (baseIndex < 0)
            {
                ...
                {
                    if (Global.Instance.AllGear[g] is MiniCannon &&
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[SiegeCannon] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[SiegeCannon] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[SiegeCannon] OnClose stamp: {ex.Message}");
            }
        }
