...
///  2. Postfix: stamp our GearInfo + ExampleWeaponBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[WeaponTemplate] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[WeaponTemplate] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[WeaponTemplate] Cannot remap spawn — base gun AllGear index unknown. " +
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[WeaponTemplate] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[WeaponTemplate] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[WeaponTemplate] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[WeaponTemplate] Catalog gear missing during stamp.");
            return;
        }
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            ExampleWeaponBehaviour behaviour = live.gameObject.GetComponent<ExampleWeaponBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<ExampleWeaponBehaviour>();

            ExampleWeaponBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<ExampleWeaponBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[WeaponTemplate] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }
        ...
        // Write the catalog id so the next boot restores our weapon, not CartridgeSMG/BounceShotgun.
        PersistEquippedCatalogId(catalog, slot);

        SparrohPlugin.Logger?.LogInfo(
            $"[WeaponTemplate] Rebound Gear[{slot}] → {SparrohPlugin.ExampleGearApiName} " +
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[WeaponTemplate] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[WeaponTemplate] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...

    {
        ...
        if (gear.Info != null && gear.Info.APIName == SparrohPlugin.ExampleGearApiName)
            return true;

        if (gear.Info != null && gear.Info.ID == SparrohPlugin.ExampleGearId)
            return true;
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[WeaponTemplate] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[WeaponTemplate] Safe OnCloseCallback failed: {ex}");
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[WeaponTemplate] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[WeaponTemplate] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[WeaponTemplate] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[WeaponTemplate] OnClose stamp: {ex.Message}");
            }
        }
