...
///  2. Postfix: stamp our GearInfo + RapturesChosenBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[RapturesChosen] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        //SparrohPlugin.Logger?.LogInfo(
            //$"[RapturesChosen] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[RapturesChosen] Cannot remap spawn — base gun AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        SparrohPlugin.Logger?.LogInfo(
            $"[RapturesChosen] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[RapturesChosen] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[RapturesChosen] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[RapturesChosen] Catalog gear missing during stamp.");
            return;
        }
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            RapturesChosenBehaviour behaviour = live.gameObject.GetComponent<RapturesChosenBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<RapturesChosenBehaviour>();

            RapturesChosenBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<RapturesChosenBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[RapturesChosen] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        // Re-apply catalog GunData baseline and ensure identity hooks.
        if (live is Gun liveGun)
        {
            WeaponRegistration.ApplyRapturesChosenStats(liveGun, SparrohPlugin.Logger);
            if (RapturesChosenBehaviour.TryGet(live, out RapturesChosenBehaviour b))
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[RapturesChosen] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[RapturesChosen] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[RapturesChosen] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[RapturesChosen] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[RapturesChosen] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[RapturesChosen] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[RapturesChosen] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[RapturesChosen] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[RapturesChosen] OnClose stamp: {ex.Message}");
            }
        }
