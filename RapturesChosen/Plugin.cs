...
/// <summary>
/// Rapture's Chosen — dual-mode shock lance primary for Mycopunk.
/// Phase 0: registration (clone Shocklance). Phase 1: coil + baseline RMB Auger.
/// Vanilla Shocklance left untouched.
/// </summary>
[BepInPlugin(PluginGUID, PluginName, PluginVersion)]
[MycoMod(null, ModFlags.IsSandbox)]
public class SparrohPlugin : BaseUnityPlugin
{
    public const string PluginGUID = "sparroh.raptureschosen";
    public const string PluginName = "RapturesChosen";
    public const string PluginVersion = "0.1.0";

    /// <summary>Stable numeric GearInfo.ID. High unique range to avoid vanilla / other mods.</summary>
    public const int GearId = 92500;


    /// <summary>Value of GearInfo.APIName — used by FindGear / AllGear scans.</summary>
    public const string GearApiName = "raptures_chosen";

    public const string GearDisplayName = "Rapture's Chosen";
    public const string GearDescription =
        "Dual-mode shock lance. Charge M1 for a piercing coil; hold RMB to charge and release Auger. " +
        "Shock on both modes. ADS disabled.";

    /// <summary>Vanilla gun type to clone (coil spiral + Auger DNA + NGO spawn validity).</summary>
    ...
    private void Awake()
    {
        ...
        RapturesChosenCombatHooks.Apply(_harmony);
    ...
    internal void TryRegisterGear(string reason)
    {
        ...
        {
            Logger.LogDebug($"[RapturesChosen] Global.AllGear not ready yet ({reason}).");
            return;
        }
        ...
        try
        {
            ...
            //Logger.LogInfo(
                //$"[RapturesChosen] Registered gear '{GearDisplayName}' " +
                //$"(api={GearApiName}, id={GearId}) via {reason}.");
        }
        catch (Exception ex)
        {
            Logger.LogError($"[RapturesChosen] Gear registration failed: {ex}");
        }
    }
...
/// <summary>
/// Keep Rapture's Chosen alive across save load.
///
...
internal static class PlayerDataPersistenceHooks
{
    ...
    private static void Postfix()
    {
        try
        {
            ...
            if (gear == null)
            {
                SparrohPlugin.Logger?.LogWarning("[RapturesChosen] Persistence: gear missing after OnAwake.");
                return;
            }
            ...
            if (gd != null)
            {
                ...
                //SparrohPlugin.Logger?.LogInfo(
                    //$"[RapturesChosen] Persistence OK: level={gd.Level} unlocked={gd.IsUnlocked} " +
        ...
        {
            SparrohPlugin.Logger?.LogError($"[RapturesChosen] Persistence postfix failed: {ex}");
        }
    }
    ...
    private static void TryEnsureSavedLoadoutPointsAtCatalog(IUpgradable gear)
    {
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[RapturesChosen] Save loadout references catalog id={id} " +
