...
/// <summary>
/// Creates and registers Stalker's Blade as a GearType.Melee kit by cloning vanilla MeleeGear.
...
public static class WeaponRegistration
{
        ...
        out IUpgradable registeredGear)
    {
        ...
        StalkersBladeBehaviour behaviour = clone.GetComponent<StalkersBladeBehaviour>();
        if (behaviour == null)
            behaviour = clone.AddComponent<StalkersBladeBehaviour>();
        behaviour.InitializeAsPrefab(description);
        behaviour.CapturePrefabSnapshot();

        ApplyBladeStats(cloneMelee);
    ...
    public static void ApplyBladeStats(MeleeGear melee)
    {
        if (melee == null)
            return;

        ref GunData gun = ref melee.GunData;
        gun.damage = StalkersBladeBalance.Damage;
        gun.damageEffect = StalkersBladeBalance.DamageEffect;
        gun.damageEffectAmount = StalkersBladeBalance.DamageEffectAmount;
        gun.bulletMagnetismTarget = StalkersBladeBalance.Size;
        gun.hitForce = StalkersBladeBalance.HitForce;
        gun.hitVFXSize = StalkersBladeBalance.HitVfxSize;
        StalkersBladeBalance.ApplyReach(ref gun, StalkersBladeBalance.Reach);

        ref CooldownData cd = ref melee.CooldownData;
        cd.rechargeDuration = Mathf.Max(0.05f, StalkersBladeBalance.Cooldown);
    }
