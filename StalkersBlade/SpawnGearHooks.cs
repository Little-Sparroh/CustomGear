...
/// then stamp Stalker's Blade identity + behaviour + balance stats.
/// </summary>
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[StalkersBlade] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        //SparrohPlugin.Logger?.LogInfo(
            //$"[StalkersBlade] Patched {target.Name} ({ps.Length} params) with {prefix.Name}.");
    }
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[StalkersBlade] Cannot remap spawn — base MeleeGear AllGear index unknown.");
            return;
        }

        SparrohPlugin.Logger?.LogInfo(
            $"[StalkersBlade] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[StalkersBlade] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[StalkersBlade] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[StalkersBlade] Catalog gear missing during stamp.");
            return;
        }
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            StalkersBladeBehaviour behaviour = live.gameObject.GetComponent<StalkersBladeBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<StalkersBladeBehaviour>();

            StalkersBladeBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<StalkersBladeBehaviour>();
        ...
        try
        {
            ...
            if (live is MeleeGear melee2)
                WeaponRegistration.ApplyBladeStats(melee2);
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogError(
                $"[StalkersBlade] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[StalkersBlade] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
