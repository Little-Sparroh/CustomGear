...
///
/// Strategy:
///  1. Prefix remaps allGearIndex to the vanilla AcceleratorGun index.
///  2. Postfix stamps our GearInfo + OverdriverBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[Overdriver] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[Overdriver] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[Overdriver] Cannot remap spawn — base AcceleratorGun AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        SparrohPlugin.Logger?.LogInfo(
            $"[Overdriver] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static int ResolveBaseIndex()
    {
        ...
        if (baseIndex >= 0 && baseIndex < Global.Instance.AllGear.Length)
        {
            ...
            if (at != null && !IsOurCatalogGear(at) && at is AcceleratorGun)
                return baseIndex;
        }
        ...
        // Prefer a real AcceleratorGun that is not our catalog entry.
        for (int i = 0; i < Global.Instance.AllGear.Length; i++)
        {
            if (Global.Instance.AllGear[i] is AcceleratorGun &&
                !IsOurCatalogGear(Global.Instance.AllGear[i]))
            {
                WeaponRegistration.SetBaseAllGearIndex(i);
                return i;
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[Overdriver] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[Overdriver] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[Overdriver] Catalog gear missing during stamp.");
            return;
        }
    ...
    /// <summary>
    /// After NGO spawns vanilla AcceleratorGun, rebind live instance to catalog identity
    /// and re-run ApplyUpgrades so GunData restores from our Overdriver baseline.
    /// </summary>
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            OverdriverBehaviour behaviour = live.gameObject.GetComponent<OverdriverBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<OverdriverBehaviour>();

            OverdriverBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<OverdriverBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[Overdriver] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        // Ensure Overdriver balance sticks even if ApplyUpgrades path is partial.
        if (live is Gun liveGun && catalog is Gun catalogGun)
        {
            if (Mathf.Abs(liveGun.GunData.damage - catalogGun.GunData.damage) > 0.01f ||
                ...
                liveGun.GunData.automatic != catalogGun.GunData.automatic)
            {
                WeaponRegistration.ApplyOverdriverStats(liveGun);
            }

            var odLive = liveGun.GetComponent<OverdriverBehaviour>();
            if (odLive != null)
                odLive.OnUpgradesApplied(liveGun);
        }

        PersistEquippedCatalogId(catalog, slot);

        SparrohPlugin.Logger?.LogInfo(
            $"[Overdriver] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[Overdriver] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[Overdriver] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[Overdriver] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[Overdriver] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[Overdriver] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            if (baseIndex < 0)
            {
                ...
                {
                    if (Global.Instance.AllGear[g] is AcceleratorGun &&
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[Overdriver] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[Overdriver] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[Overdriver] OnClose stamp: {ex.Message}");
            }
        }
