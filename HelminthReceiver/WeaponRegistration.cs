...
///  5. Attaches <see cref="HelminthBehaviour"/> for custom data / upgrade hooks
...
public static class WeaponRegistration
{
        ...
        out IUpgradable registeredGear)
    {
        ...
        // Custom behaviour host — upgrades / combat hooks resolve via GetComponent.
        HelminthBehaviour behaviour = clone.GetComponent<HelminthBehaviour>();
        if (behaviour == null)
            behaviour = clone.AddComponent<HelminthBehaviour>();
        behaviour.InitializeAsPrefab(description);

        ApplyHelminthBaseline(cloneGun, behaviour, log);
    ...
    /// <summary>
    /// Design §3.2 mid-band pulse rifle on the catalog / live CartridgeSMG clone.
    /// Values from <see cref="HelminthBalance"/>. Vitality is the real magazine;
    /// vanilla ammo is disabled / mirrored.
    /// </summary>
    public static void ApplyHelminthStats(Gun gun, HelminthBehaviour behaviour = null, ManualLogSource log = null)
    {
        if (gun == null)
            return;
        ...
        // --- Core combat ---
        g.damage = HelminthBalance.Damage;
        g.damageEffect = HelminthBalance.DamageEffect;
        g.damageEffectAmount = HelminthBalance.DamageEffectAmount;
        g.fireInterval = HelminthBalance.FireInterval;
        g.fireAnimationSpeedMultiplier = Mathf.Max(
            g.fireAnimationSpeedMultiplier, HelminthBalance.FireAnimationSpeedMultiplier);
        g.automatic = HelminthBalance.Automatic;
        g.bulletsPerShot = HelminthBalance.BulletsPerShot;
        g.burstSize = HelminthBalance.BurstSize;
        g.burstFireInterval = HelminthBalance.BurstFireInterval;
        g.useAmmoOnFire = HelminthBalance.UseAmmoOnFire;
        g.doesEachBulletInShotRemoveAmmo = HelminthBalance.DoesEachBulletInShotRemoveAmmo;
        g.doesEachBulletInShotTriggerEffects = HelminthBalance.DoesEachBulletInShotTriggerEffects;

        // --- Magazine / reserves (Vitality owns real ammo) ---
        g.hasLimitedAmmo = HelminthBalance.HasLimitedAmmo;
        g.ammoCapacity = HelminthBalance.AmmoCapacity;
        g.ammoCollectMultiplier = HelminthBalance.AmmoCollectMultiplier;
        g.storedAmmoCollectMultiplier = HelminthBalance.StoredAmmoCollectMultiplier;
        g.ammoGenerationEfficiency = HelminthBalance.AmmoGenerationEfficiency;
        g.useAmmoWhileFiringInterval = HelminthBalance.UseAmmoWhileFiringInterval;
        g.refillAmmoOnReload = HelminthBalance.RefillAmmoOnReload;
        g.reloadDuration = HelminthBalance.ReloadDuration;
        g.autoReloadWhenEmpty = HelminthBalance.AutoReloadWhenEmpty;

        int shots = behaviour != null
            ? behaviour.WholeShotsCapacity()
            : HelminthBalance.MagazineSizeFromVitality;
        g.magazineSize = Mathf.Max(1, shots);

        // --- Projectile ---
        g.bulletSpeed = HelminthBalance.BulletSpeed;
        g.bulletGravity = HelminthBalance.BulletGravity;
        g.maxBounces = HelminthBalance.MaxBounces;
        g.bulletMagnetismSurface = HelminthBalance.BulletMagnetismSurface;
        g.bulletMagnetismTarget = HelminthBalance.BulletMagnetismTarget;
        g.hitForce = HelminthBalance.HitForce;
        g.hitVFXSize = Mathf.Max(g.hitVFXSize, HelminthBalance.HitVfxSize);
        g.bulletShakeTranslation = Mathf.Max(
            g.bulletShakeTranslation, HelminthBalance.BulletShakeTranslation);
        g.bulletShakeRotation = Mathf.Max(
            g.bulletShakeRotation, HelminthBalance.BulletShakeRotation);

        // --- Range / falloff ---
        g.rangeData.falloffStartDistance = HelminthBalance.FalloffStartDistance;
        g.rangeData.falloffEndDistance = HelminthBalance.FalloffEndDistance;
        g.rangeData.maxDamageRange = HelminthBalance.MaxDamageRange;
        g.rangeData.maxFalloffDamageMultiplier = HelminthBalance.MaxFalloffDamageMultiplier;

        // --- Spread ---
        g.spreadData.spreadType = HelminthBalance.SpreadType;
        g.spreadData.spreadSize = HelminthBalance.SpreadSize;
        g.firstShotSpreadMultiplier = HelminthBalance.FirstShotSpreadMultiplier;

        // --- Recoil (soft organic kick) ---
        g.recoilData.recoilX = HelminthBalance.RecoilX;
        g.recoilData.recoilY = HelminthBalance.RecoilY;
        g.recoilData.recoilZ = HelminthBalance.RecoilZ;
        g.recoilData.maxRecoilZ = HelminthBalance.MaxRecoilZ;
        g.recoilData.translateZ = HelminthBalance.TranslateZ;
        g.recoilData.maxTranslateZ = HelminthBalance.MaxTranslateZ;
        g.recoilData.aimTranslateMultiplier = HelminthBalance.AimTranslateMultiplier;
        g.recoilData.recoilSpeed = HelminthBalance.RecoilSpeed;
        g.recoilData.recoilRecoverySpeed = HelminthBalance.RecoilRecoverySpeed;
        g.recoilData.translateSpeed = HelminthBalance.TranslateSpeed;
        g.recoilData.translateRecoverySpeed = HelminthBalance.TranslateRecoverySpeed;
        g.recoilData.recoilTargetDecaySpeed = HelminthBalance.RecoilTargetDecaySpeed;
        g.recoilData.aimRecoilMultiplier = HelminthBalance.AimRecoilMultiplier;

        // --- Charge (disabled on base) ---
        g.chargeData.duration = HelminthBalance.ChargeDuration;
        g.chargeData.coolDownSpeed = HelminthBalance.ChargeCoolDownSpeed;
        g.chargeData.fireWhenFullyCharged = HelminthBalance.ChargeFireWhenFullyCharged;
        g.chargeData.fireOnRelease = HelminthBalance.ChargeFireOnRelease;
        g.chargeData.canFireWhileCharging = HelminthBalance.ChargeCanFireWhileCharging;
        g.chargeData.time = 0f;

        // --- Fire constraints ---
        g.fireConstraints.canFireWhileSprinting = HelminthBalance.CanFireWhileSprinting;
        g.fireConstraints.canFireWhileSliding = HelminthBalance.CanFireWhileSliding;
        g.fireConstraints.canAimWhileSliding = HelminthBalance.CanAimWhileSliding;
        g.fireConstraints.canAimWhileReloading = HelminthBalance.CanAimWhileReloading;
        g.fireConstraints.canReloadWhileSprinting = HelminthBalance.CanReloadWhileSprinting;

        // --- ADS ---
        gun.IsAimEnabled = HelminthBalance.IsAimEnabled;
        gun.AimFOV = HelminthBalance.AimFov;
        TrySetAimTransitionDuration(gun, HelminthBalance.AimTransitionDuration);

        //log?.LogInfo(
            //$"[WeaponRegistration] Helminth stats: dmg={g.damage} interval={g.fireInterval} " +
            //$"rpm≈{60f / Mathf.Max(0.01f, g.fireInterval):0} auto={g.automatic} " +
            //$"magMirror={g.magazineSize} Vmax={behaviour?.WeaponData.maxVitality ?? HelminthBalance.MaxVitality} " +
            //$"Vcost={behaviour?.WeaponData.vitalityPerShot ?? HelminthBalance.VitalityPerShot} " +
            //$"limitedAmmo={g.hasLimitedAmmo} speed={g.bulletSpeed} grav={g.bulletGravity}.");
    }

    /// <summary>Catalog registration entry — full baseline stamp from <see cref="HelminthBalance"/>.</summary>
    private static void ApplyHelminthBaseline(Gun cloneGun, HelminthBehaviour behaviour, ManualLogSource log)
    {
        ApplyHelminthStats(cloneGun, behaviour, log);
    }
...
public static class ModelImportHooks
{
    ...
    public static void ApplyPlaceholderHooks(GameObject gearRoot, ManualLogSource log)
    {
        ...
        log?.LogDebug("[ModelImportHooks] Placeholder only — using vanilla CartridgeSMG visuals (Helminth).");
