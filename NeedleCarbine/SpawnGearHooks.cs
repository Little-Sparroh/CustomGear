...
///  2. Postfix: stamp our GearInfo + NeedleCarbineBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[NeedleCarbine] Could not find Player.SpawnGear_Server to patch.");
        ...
        //SparrohPlugin.Logger?.LogInfo(
            //$"[NeedleCarbine] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[NeedleCarbine] Cannot remap spawn — base gun AllGear index unknown. " +
        ...
        SparrohPlugin.Logger?.LogInfo(
            $"[NeedleCarbine] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[NeedleCarbine] Post-spawn stamp failed: {ex}");
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[NeedleCarbine] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[NeedleCarbine] Catalog gear missing during stamp.");
            return;
        }
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            NeedleCarbineBehaviour behaviour = live.gameObject.GetComponent<NeedleCarbineBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<NeedleCarbineBehaviour>();

            NeedleCarbineBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<NeedleCarbineBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[NeedleCarbine] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        // 4) Re-assert balance + laser suppress after ApplyUpgrades.
        if (live is Gun liveGun)
        {
            if (catalog is Gun catalogGun && NeedsNeedleStatRepair(liveGun, catalogGun))
                WeaponRegistration.ApplyNeedleCarbineStats(liveGun, SparrohPlugin.Logger);

            liveGun.IsAimEnabled = NcBalance.IsAimEnabled;
            liveGun.GunData.automatic = NcBalance.Automatic;

            if (NeedleCarbineBehaviour.TryGet(liveGun, out var nc))
                nc.OnUpgradesApplied(liveGun);

            NeedleCarbineBehaviour.SuppressLaserMode(liveGun);
        }

        // Critical: SpawnGear remaps catalog → base gun for NGO. Vanilla save fields
        // often record the base id from that spawn. Write the catalog id.
        PersistEquippedCatalogId(catalog, slot);

        SparrohPlugin.Logger?.LogInfo(
            $"[NeedleCarbine] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
            $"(Info={live.Info?.APIName}, Prefab={(live.Prefab as Component)?.name}).");
    }

    private static bool NeedsNeedleStatRepair(Gun live, Gun catalog)
    {
        if (live == null || catalog == null)
            return false;

        if (catalog.GunData.magazineSize == NcBalance.MagazineSize &&
            live.GunData.magazineSize != NcBalance.MagazineSize)
            return true;
        ...
        if (live.IsAimEnabled != NcBalance.IsAimEnabled)
            return true;
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogInfo(
                $"[NeedleCarbine] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[NeedleCarbine] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[NeedleCarbine] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[NeedleCarbine] Safe OnCloseCallback failed: {ex}");
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[NeedleCarbine] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[NeedleCarbine] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogInfo(
                $"[NeedleCarbine] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[NeedleCarbine] OnClose stamp: {ex.Message}");
