...
///  5. Attaches <see cref="NeedleCarbineBehaviour"/> for custom data / combat hooks
...
public static class WeaponRegistration
{
        ...
        out IUpgradable registeredGear)
    {
        ...
        if (existing != null)
        {
            ...
            if (existing is Gun existingGun)
                ApplyNeedleCarbineStats(existingGun, log);
        ...
        NeedleCarbineBehaviour behaviour = clone.GetComponent<NeedleCarbineBehaviour>();
        if (behaviour == null)
            behaviour = clone.AddComponent<NeedleCarbineBehaviour>();
        behaviour.InitializeAsPrefab(description);

        ApplyNeedleCarbineStats(cloneGun, log);
    ...
    /// <summary>
    /// Rewrites GunData / aim / laser-disable fields from <see cref="NcBalance"/>.
    /// Applied to the catalog clone and re-asserted on live spawn after ApplyUpgrades.
    /// </summary>
    public static void ApplyNeedleCarbineStats(Gun gun, ManualLogSource log = null)
    {
        if (gun == null)
            return;
        ...
        // --- Core combat ---
        g.damage = NcBalance.Damage;
        g.damageEffect = NcBalance.DamageEffect;
        g.damageEffectAmount = NcBalance.DamageEffectAmount;
        g.fireInterval = NcBalance.FireInterval;
        g.fireAnimationSpeedMultiplier = NcBalance.FireAnimationSpeedMultiplier;
        g.automatic = NcBalance.Automatic;
        g.bulletsPerShot = NcBalance.BulletsPerShot;
        g.burstSize = NcBalance.BurstSize;
        g.burstFireInterval = NcBalance.BurstFireInterval;
        g.useAmmoOnFire = NcBalance.UseAmmoOnFire;
        g.doesEachBulletInShotRemoveAmmo = NcBalance.DoesEachBulletInShotRemoveAmmo;
        g.doesEachBulletInShotTriggerEffects = NcBalance.DoesEachBulletInShotTriggerEffects;
        g.hitForce = NcBalance.HitForce;
        g.hitVFXSize = NcBalance.HitVfxSize;

        // --- Magazine / reserves ---
        g.magazineSize = NcBalance.MagazineSize;
        g.hasLimitedAmmo = NcBalance.HasLimitedAmmo;
        g.ammoCapacity = NcBalance.AmmoCapacity;
        g.ammoCollectMultiplier = NcBalance.AmmoCollectMultiplier;
        g.storedAmmoCollectMultiplier = NcBalance.StoredAmmoCollectMultiplier;
        g.ammoGenerationEfficiency = NcBalance.AmmoGenerationEfficiency;
        g.useAmmoWhileFiringInterval = NcBalance.UseAmmoWhileFiringInterval;
        g.refillAmmoOnReload = NcBalance.RefillAmmoOnReload;
        g.reloadDuration = NcBalance.ReloadDuration;
        g.autoReloadWhenEmpty = NcBalance.AutoReloadWhenEmpty;

        // --- Projectile (Scout primary path) ---
        g.bulletSpeed = NcBalance.BulletSpeed;
        g.bulletGravity = NcBalance.BulletGravity;
        g.maxBounces = NcBalance.MaxBounces;
        g.bulletMagnetismSurface = NcBalance.BulletMagnetismSurface;
        g.bulletMagnetismTarget = NcBalance.BulletMagnetismTarget;
        g.bulletShakeTranslation = NcBalance.BulletShakeTranslation;
        g.bulletShakeRotation = NcBalance.BulletShakeRotation;

        // --- Range / falloff ---
        g.rangeData.falloffStartDistance = NcBalance.FalloffStartDistance;
        g.rangeData.falloffEndDistance = NcBalance.FalloffEndDistance;
        g.rangeData.maxDamageRange = NcBalance.MaxDamageRange;
        g.rangeData.maxFalloffDamageMultiplier = NcBalance.MaxFalloffDamageMultiplier;

        // --- Spread ---
        g.spreadData.spreadType = NcBalance.SpreadType;
        g.spreadData.spreadSize = NcBalance.SpreadSize;
        g.firstShotSpreadMultiplier = NcBalance.FirstShotSpreadMultiplier;

        // --- Recoil ---
        g.recoilData.recoilX = NcBalance.RecoilX;
        g.recoilData.recoilY = NcBalance.RecoilY;
        g.recoilData.recoilZ = NcBalance.RecoilZ;
        g.recoilData.maxRecoilZ = NcBalance.MaxRecoilZ;
        g.recoilData.translateZ = NcBalance.TranslateZ;
        g.recoilData.maxTranslateZ = NcBalance.MaxTranslateZ;
        g.recoilData.aimTranslateMultiplier = NcBalance.AimTranslateMultiplier;
        g.recoilData.recoilSpeed = NcBalance.RecoilSpeed;
        g.recoilData.recoilRecoverySpeed = NcBalance.RecoilRecoverySpeed;
        g.recoilData.translateSpeed = NcBalance.TranslateSpeed;
        g.recoilData.translateRecoverySpeed = NcBalance.TranslateRecoverySpeed;
        g.recoilData.recoilTargetDecaySpeed = NcBalance.RecoilTargetDecaySpeed;
        g.recoilData.aimRecoilMultiplier = NcBalance.AimRecoilMultiplier;

        // --- Charge disabled ---
        g.chargeData.duration = NcBalance.ChargeDuration;
        g.chargeData.coolDownSpeed = NcBalance.ChargeCoolDownSpeed;
        g.chargeData.fireWhenFullyCharged = NcBalance.ChargeFireWhenFullyCharged;
        g.chargeData.fireOnRelease = NcBalance.ChargeFireOnRelease;
        g.chargeData.canFireWhileCharging = NcBalance.ChargeCanFireWhileCharging;
        g.chargeData.time = 0f;

        // --- Fire constraints ---
        g.fireConstraints.canFireWhileSprinting = NcBalance.CanFireWhileSprinting;
        g.fireConstraints.canFireWhileSliding = NcBalance.CanFireWhileSliding;
        g.fireConstraints.canAimWhileSliding = NcBalance.CanAimWhileSliding;
        g.fireConstraints.canAimWhileReloading = NcBalance.CanAimWhileReloading;
        g.fireConstraints.canReloadWhileSprinting = NcBalance.CanReloadWhileSprinting;

        // --- ADS off (RMB = Extract) ---
        gun.IsAimEnabled = NcBalance.IsAimEnabled;
        gun.AimFOV = NcBalance.AimFov;
        TrySetAimTransitionDuration(gun, NcBalance.AimTransitionDuration);

        // --- Starve Scout laser so dual-mode cannot engage ---
        if (gun is ScoutLaserRifle scout)
        {
            ref ScoutLaserRifle.LaserGunData laser = ref scout.LaserData;
            laser.laserChargeCapacity = NcBalance.LaserChargeCapacity;
            laser.laserChargeOnHit = NcBalance.LaserChargeOnHit;
            laser.laserChargeUsePerSecond = NcBalance.LaserChargeUsePerSecond;
            laser.minLaserCharge = NcBalance.MinLaserCharge;
            laser.laserFireInterval = NcBalance.LaserFireInterval;
            laser.laserDamage = new DamageData(NcBalance.LaserDamage, EffectType.Normal, 0f, DamageFlags.None);
            laser.laserAmmoUseInterval = 0f;
            laser.laserAmmoRefill = 0f;
            ...
            catch { /* offline / not spawned */ }

            //log?.LogInfo(
                //$"[WeaponRegistration] Applied Needle Carbine stats: dmg={g.damage}, " +
                ...
                //$"aim={NcBalance.IsAimEnabled}, laserDisabled=true.");
        }
        else
        {
            log?.LogWarning(
                "[WeaponRegistration] Gun is not ScoutLaserRifle — laser disable skipped.");
            log?.LogInfo(
                $"[WeaponRegistration] Applied Needle Carbine GunData: dmg={g.damage}, " +
                $"mag={g.magazineSize}, automatic={g.automatic}, aim={NcBalance.IsAimEnabled}.");
        }
    }
