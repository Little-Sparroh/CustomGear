...
/// <summary>
/// Fixes equipping runtime-cloned Honey Jar.
///
...
///  2. Postfix stamps Honey Jar identity onto the live instance (with retry if async).
///  3. Persist catalog id to PlayerData.grenadeID so next boot restores Honey Jar, not Incendiary.
/// </summary>
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            HoneyJarPlugin.Logger?.LogError("[HoneyJar] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        //HoneyJarPlugin.Logger?.LogInfo(
            //$"[HoneyJar] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            HoneyJarPlugin.Logger?.LogError(
                "[HoneyJar] Cannot remap spawn — base Incendiary AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        HoneyJarPlugin.Logger?.LogInfo(
            $"[HoneyJar] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        try
        {
            ...
            {
                HoneyJarPlugin plugin = HoneyJarPlugin.Instance;
                if (plugin != null)
        ...
        {
            HoneyJarPlugin.Logger?.LogError($"[HoneyJar] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static IEnumerator StampWhenReady(Player player, int slot)
    {
        for (int attempt = 0; attempt < 30; attempt++)
        {
            ...
            {
                HoneyJarPlugin.Logger?.LogWarning(
                    $"[HoneyJar] Deferred stamp attempt {attempt}: {ex.Message}");
            }
        }

        HoneyJarPlugin.Logger?.LogWarning(
            $"[HoneyJar] Deferred stamp gave up on Gear[{slot}] after retries.");
    }
    ...
    private static bool TryStampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        IUpgradable catalog = HoneyJarPlugin.CustomGrenadePrefab
            ?? GrenadeRegistration.CatalogGear
            ?? FindCatalogInAllGear();

        if (catalog == null)
        {
            HoneyJarPlugin.Logger?.LogWarning("[HoneyJar] Catalog gear missing during stamp.");
            return false;
        }
    ...
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            HoneyJarBehaviour behaviour = live.gameObject.GetComponent<HoneyJarBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<HoneyJarBehaviour>();

            HoneyJarBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<HoneyJarBehaviour>();

            behaviour.InitializeAsPrefab(
                templateBehaviour != null ? templateBehaviour.Description : HoneyJarPlugin.GearDescription);

            if (templateBehaviour != null)
                behaviour.CopySnapshotFrom(templateBehaviour);
        }

        // Always stamp HoneyJarBalance onto the live Incendiary instance before upgrades
        ...
        {
            HoneyJarPlugin.Logger?.LogWarning(
                $"[HoneyJar] Post-stamp ApplyUpgrades failed on Gear[{slot}]: {ex.Message}");
        }

        PersistEquippedCatalogId(catalog, slot);

        HoneyJarPlugin.Logger?.LogInfo(
            $"[HoneyJar] Rebound Gear[{slot}] → {HoneyJarPlugin.GearApiName} " +
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            GrenadeRegistration.EnsureGearData(catalog, autoUnlock: true, HoneyJarPlugin.Logger);
            ...
            HoneyJarPlugin.Logger?.LogInfo(
                $"[HoneyJar] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            HoneyJarPlugin.Logger?.LogWarning($"[HoneyJar] PersistEquippedCatalogId: {ex.Message}");
        }
    }
    ...
    internal static bool IsOurCatalogGear(IUpgradable gear)
    {
        ...
        if (gear == HoneyJarPlugin.CustomGrenadePrefab || gear == GrenadeRegistration.CatalogGear)
            return true;

        if (gear.Info != null && gear.Info.APIName == HoneyJarPlugin.GearApiName)
            return true;

        if (gear.Info != null && gear.Info.ID == HoneyJarPlugin.GearId)
            return true;
...
/// and stamps/persists Honey Jar identity after close when our catalog is selected.
/// </summary>
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            HoneyJarPlugin.Logger?.LogError(
                "[HoneyJar] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static void OnCloseCallbackPostfix()
    {
        try
        {
            ...
            IUpgradable catalog = HoneyJarPlugin.ResolveRegisteredGear();
            if (catalog == null)
                return;
            ...
            bool isOurs =
                ...
                (live.Info != null && live.Info.APIName == HoneyJarPlugin.GearApiName);
        ...
        {
            HoneyJarPlugin.Logger?.LogWarning($"[HoneyJar] OnClose stamp/persist: {ex.Message}");
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            HoneyJarPlugin.Logger?.LogError(
                "[HoneyJar] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
