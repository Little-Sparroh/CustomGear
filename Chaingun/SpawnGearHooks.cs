...
///
/// Strategy:
///  1. Prefix remaps allGearIndex to the vanilla MiniCannon base gun index.
///  2. Postfix stamps our GearInfo + ChaingunBehaviour onto the live instance.
...
internal static class SpawnGearHooks
{
    ...
    public static void Apply(Harmony harmony)
    {
        ...
        if (target == null)
        {
            SparrohPlugin.Logger?.LogError("[Chaingun] Could not find Player.SpawnGear_Server to patch.");
            return;
        }
        ...
        SparrohPlugin.Logger?.LogDebug(
            $"[Chaingun] Patched {target.DeclaringType.FullName}.{target.Name} " +
    ...
    private static void TryRemap(int slot, ref int allGearIndex)
    {
        ...
        if (baseIndex < 0)
        {
            SparrohPlugin.Logger?.LogError(
                "[Chaingun] Cannot remap spawn — base gun AllGear index unknown. " +
                "Aborting custom equip to avoid NRE.");
            return;
        }

        SparrohPlugin.Logger?.LogDebug(
            $"[Chaingun] Remap SpawnGear slot={slot} index {allGearIndex} → base {baseIndex} " +
    ...
    private static void SpawnGearServerPostfix(Player __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[Chaingun] Post-spawn stamp failed: {ex}");
        }
    }
    ...
    private static void StampCustomIdentityOnSlot(Player player, int slot)
    {
        ...
        if (live == null)
        {
            SparrohPlugin.Logger?.LogWarning(
                $"[Chaingun] Post-spawn Gear[{slot}] is null — base spawn may have failed.");
            return;
        }
        ...
        if (catalog == null)
        {
            SparrohPlugin.Logger?.LogWarning("[Chaingun] Catalog gear missing during stamp.");
            return;
        }
    ...
    /// <summary>
    /// After NGO spawns the vanilla MiniCannon, rebind the live instance to our catalog identity
    /// and re-run IGear.ApplyUpgrades so GunData restores from our chaingun catalog baseline.
    /// </summary>
    internal static void RebindLiveGear(IGear live, IUpgradable catalog, int slot = -1)
    {
        ...
        {
            ChaingunBehaviour behaviour = live.gameObject.GetComponent<ChaingunBehaviour>();
            if (behaviour == null)
                behaviour = live.gameObject.AddComponent<ChaingunBehaviour>();

            ChaingunBehaviour templateBehaviour = null;
            if (catalog is Component cc)
                templateBehaviour = cc.GetComponent<ChaingunBehaviour>();
        ...
        {
            SparrohPlugin.Logger?.LogError(
                $"[Chaingun] ApplyUpgrades after rebind failed (slot={slot}): {ex}");
        }

        if (live is Gun liveGun && catalog is Gun catalogGun)
        {
            // Re-apply if ApplyUpgrades left vanilla MiniCannon damage/profile.
            if (Mathf.Abs(liveGun.GunData.damage - catalogGun.GunData.damage) > 0.5f ||
                liveGun.GunData.magazineSize != catalogGun.GunData.magazineSize)
            {
                WeaponRegistration.ApplyChaingunStats(liveGun);
            }

            if (ChaingunBehaviour.TryGet(liveGun, out var cgLive))
                cgLive.OnUpgradesApplied(liveGun);

            WeaponRegistration.EnsureRailBullet(liveGun);
            ChaingunBehaviour.DisableVanillaMiniCannonSpinUp(liveGun);
        }

        PersistEquippedCatalogId(catalog, slot);

        SparrohPlugin.Logger?.LogDebug(
            $"[Chaingun] Rebound Gear[{slot}] → {SparrohPlugin.GearApiName} " +
    ...
    internal static void PersistEquippedCatalogId(IUpgradable catalog, int slot)
    {
        ...
        try
        {
            ...
            SparrohPlugin.Logger?.LogDebug(
                $"[Chaingun] Persisted equipped id={id} slot={slot} " +
                $"(w1={PlayerData.Instance.weapon1ID} w2={PlayerData.Instance.weapon2ID} g={PlayerData.Instance.grenadeID}).");
        }
        catch (Exception ex)
        {
            SparrohPlugin.Logger?.LogWarning($"[Chaingun] PersistEquippedCatalogId: {ex.Message}");
        }
    }
...
internal static class GearSelectionWindowHooks
{
    ...
    private static Exception OnOpenFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[Chaingun] GearSelectionWindow.OnOpen NRE (null Gear slot after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool OnCloseCallbackPrefix(GearSelectionWindow __instance)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError($"[Chaingun] Safe OnCloseCallback failed: {ex}");
            return true;
        }
    }
    ...
    private static Exception OnCloseFinalizer(Exception __exception)
    {
        ...
        {
            SparrohPlugin.Logger?.LogError(
                "[Chaingun] GearSelectionWindow.OnCloseCallback NRE (null Gear[i] after failed spawn).\n" +
                __exception);
            return null;
        }
        return __exception;
    ...
    private static bool SafeOnCloseCallback(GearSelectionWindow window)
    {
        ...
        for (int i = 0; i < gearEquipSlots.Length; i++)
        {
            ...
            {
                SparrohPlugin.Logger?.LogError(
                    $"[Chaingun] OnClose: no base index for custom equip slot {i}.");
                continue;
            }

            SparrohPlugin.Logger?.LogDebug(
                $"[Chaingun] OnClose equip slot {i} custom → SpawnGear baseIndex={baseIndex}.");
            ...
            {
                SparrohPlugin.Logger?.LogWarning($"[Chaingun] OnClose stamp: {ex.Message}");
            }
        }
